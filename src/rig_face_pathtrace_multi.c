...
/* [CANON] <float.h> → "rig_noext_types.h" */
 ...
 * Vectores
 * ---------------------------------------------------------------------- */
static RigPTVec3 ptv_add__rig_dup_32942a0c(RigPTVec3 a, RigPTVec3 b){RigPTVec3 r={a.x+b.x,a.y+b.y,a.z+b.z};return r;}
static RigPTVec3 ptv_sub__rig_dup_838f33b1(RigPTVec3 a, RigPTVec3 b){RigPTVec3 r={a.x-b.x,a.y-b.y,a.z-b.z};return r;}
static RigPTVec3 ptv_scale__rig_dup_9935eedb(RigPTVec3 a, float s){RigPTVec3 r={a.x*s,a.y*s,a.z*s};return r;}
static float ptv_dot__rig_dup_74a01083(RigPTVec3 a, RigPTVec3 b){return a.x*b.x+a.y*b.y+a.z*b.z;}
static float ptv_len__rig_dup_8f275071(RigPTVec3 a){return sqrtf(ptv_dot__rig_dup_74a01083(a,a));}
static RigPTVec3 ptv_norm__rig_dup_bed95a39(RigPTVec3 a){float l=ptv_len__rig_dup_8f275071(a); if(l<1e-8f){RigPTVec3 z={0,0,1};return z;} return ptv_scale__rig_dup_9935eedb(a,1.0f/l);}
/* -------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC RigPTMat4 rig_pt_mat4_identity__rig_dup_d87a8bf3(void) {
    RigPTMat4 m; memset(&m, 0, sizeof(m));
    m.m[0]=1; m.m[5]=1; m.m[10]=1; m.m[15]=1;
    return m;
}
RIGCOM_PUBLIC RigPTMat4 rig_pt_mat4_trs__rig_dup_626582d4(RigPTVec3 t, RigPTVec3 euler_deg, RigPTVec3 scale) {
    ...
    RigPTMat4 m = rig_pt_mat4_identity__rig_dup_d87a8bf3();
...
static RigPTVec3 rig_pt_mat4_mul_point__rig_dup_fc0c9d0a(const RigPTMat4 *m, RigPTVec3 p) {
    RigPTVec3 r;
...
static RigPTVec3 rig_pt_mat4_mul_dir__rig_dup_f65f478c(const RigPTMat4 *m, RigPTVec3 p) {
    RigPTVec3 r;
...
RIGCOM_PUBLIC int rig_pt_mat4_inverse__rig_dup_9eb1eb32(const RigPTMat4 *in, RigPTMat4 *out) {
    float a[4][8];
    for (int r = 0; r < 4; r++) {
...
static RigPTVec3 rig_pt_transform_normal__rig_dup_9dc6ee92(const RigPTMat4 *inverse_transform, RigPTVec3 n_local) {
    ...
    return ptv_norm__rig_dup_bed95a39(r);
}
/* -------------------------------------------------------------------------
 * AABB
 * ---------------------------------------------------------------------- */
static RigPTAABB rig_pt_aabb_union__rig_dup_bd9224ff(RigPTAABB a, RigPTAABB b) {
    RigPTAABB r;
...
static int rig_pt_aabb_hit__rig_dup_5c379b81(RigPTAABB box, RigPTRay ray, float t_max_current) {
...
static RigPTAABB rig_pt_instance_world_aabb__rig_dup_364d9df7(const RigPTInstance *inst) {
    RigPTVec3 corners[8];
    ...
    RigPTAABB world;
    world.min = world.max = rig_pt_mat4_mul_point__rig_dup_fc0c9d0a(&inst->transform, corners[0]);
    for (int i = 1; i < 8; i++) {
        RigPTVec3 p = rig_pt_mat4_mul_point__rig_dup_fc0c9d0a(&inst->transform, corners[i]);
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_pt_scene_init__rig_dup_28f5a6cf(RigPTScene *scene) {
    if (!scene) return -1;
    memset(scene, 0, sizeof(*scene));
    scene->root_node = -1;
    return 0;
}
RIGCOM_PUBLIC int rig_pt_scene_add_instance__rig_dup_4fb87de3(RigPTScene *scene, RigPTMat4 transform,
    ...
    if (rig_pt_mat4_inverse__rig_dup_9eb1eb32(&transform, &inst->inverse_transform) != 0) return -3; /* transform singular */
    ...
    inst->world_aabb = rig_pt_instance_world_aabb__rig_dup_364d9df7(inst);
    scene->instance_count++;
    return scene->instance_count - 1;
}
RIGCOM_PUBLIC int rig_pt_scene_add_light__rig_dup_790f7501(RigPTScene *scene, RigPTLight light) {
    if (!scene) return -1;
 ...
 * ---------------------------------------------------------------------- */
static int rig_pt_tlas_build_recursive__rig_dup_a83c9f08(RigPTScene *scene, int *indices, int count) {
    ...
    for (int i = 1; i < count; i++) bounds = rig_pt_aabb_union__rig_dup_bd9224ff(bounds, scene->instances[indices[i]].world_aabb);
    node->bounds = bounds;
    if (count == 1) {
        node->left = -1; node->right = -1; node->instance_index = indices[0];
        return node_idx;
    }
    RigPTVec3 extent = ptv_sub__rig_dup_838f33b1(bounds.max, bounds.min);
    int axis = 0;
    ...
    int mid = count / 2;
    int left_idx = rig_pt_tlas_build_recursive__rig_dup_a83c9f08(scene, indices, mid);
    int right_idx = rig_pt_tlas_build_recursive__rig_dup_a83c9f08(scene, indices + mid, count - mid);
...
RIGCOM_PUBLIC int rig_pt_scene_build_tlas__rig_dup_8aa5e97d(RigPTScene *scene) {
    if (!scene) return -1;
    ...
    scene->root_node = rig_pt_tlas_build_recursive__rig_dup_a83c9f08(scene, indices, scene->instance_count);
    return 0;
}
/* -------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_pt_scene_intersect__rig_dup_c52c4398(const RigPTScene *scene, RigPTRay world_ray, RigPTWorldHit *out_hit) {
    ...
    while (sp > 0) {
        ...
        if (!rig_pt_aabb_hit__rig_dup_5c379b81(node->bounds, world_ray, closest_t)) continue;
        if (node->left < 0) {
            const RigPTInstance *inst = &scene->instances[node->instance_index];
            RigPTRay local_ray;
            local_ray.origin = rig_pt_mat4_mul_point__rig_dup_fc0c9d0a(&inst->inverse_transform, world_ray.origin);
            local_ray.dir = rig_pt_mat4_mul_dir__rig_dup_f65f478c(&inst->inverse_transform, world_ray.dir);
            ...
            if (inst->intersect_fn(inst->blas_ctx, local_ray, &local_hit) && local_hit.hit && local_hit.t < closest_t) {
                ...
                out_hit->position = rig_pt_mat4_mul_point__rig_dup_fc0c9d0a(&inst->transform, local_hit.position);
                out_hit->normal = rig_pt_transform_normal__rig_dup_9dc6ee92(&inst->inverse_transform, local_hit.normal);
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_pt_scene_occluded__rig_dup_3b4a154a(const RigPTScene *scene, RigPTRay world_ray) {
    ...
    while (sp > 0) {
        int ni = stack[--sp];
        const RigPTTLASNode *node = &scene->nodes[ni];
        if (!rig_pt_aabb_hit__rig_dup_5c379b81(node->bounds, world_ray, world_ray.t_max)) continue;
        if (node->left < 0) {
            const RigPTInstance *inst = &scene->instances[node->instance_index];
            RigPTRay local_ray;
            local_ray.origin = rig_pt_mat4_mul_point__rig_dup_fc0c9d0a(&inst->inverse_transform, world_ray.origin);
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC RigPTVec3 rig_pt_scene_direct_light__rig_dup_957a6f84(const RigPTScene *scene, RigPTVec3 world_pos, RigPTVec3 world_normal) {
    ...
    for (int i = 0; i < scene->light_count; i++) {
        ...
        if (L->type == RIG_PT_LIGHT_DIRECTIONAL) {
            to_light = ptv_norm__rig_dup_bed95a39(L->position_or_dir);
        } else {
            RigPTVec3 delta = ptv_sub__rig_dup_838f33b1(L->position_or_dir, world_pos);
            dist = ptv_len__rig_dup_8f275071(delta);
            to_light = dist > 1e-6f ? ptv_scale__rig_dup_9935eedb(delta, 1.0f/dist) : (RigPTVec3){0,1,0};
        }
        float ndotl = ptv_dot__rig_dup_74a01083(world_normal, to_light);
        if (ndotl <= 0.0f) continue;
        RigPTRay shadow_ray;
        shadow_ray.origin = ptv_add__rig_dup_32942a0c(world_pos, ptv_scale__rig_dup_9935eedb(world_normal, 0.001f));
        shadow_ray.dir = to_light;
        shadow_ray.t_min = 0.0001f;
        shadow_ray.t_max = (dist == FLT_MAX) ? FLT_MAX : dist - 0.002f;
        if (rig_pt_scene_occluded__rig_dup_3b4a154a(scene, shadow_ray)) continue;
        float atten = (L->type == RIG_PT_LIGHT_POINT) ? (1.0f / (dist*dist + 1e-4f)) : 1.0f;
        float k = L->intensity * ndotl * atten;
        accum = ptv_add__rig_dup_32942a0c(accum, ptv_scale__rig_dup_9935eedb(L->color, k));
    }
    return accum;
}