 ...
 * Mapeo de huesos
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_vrm_bonemap_init__rig_dup_787f6882(RigVRMBoneMap *map) {
    if (!map) return -1;
    for (int i = 0; i < RIG_VRM_BONE_COUNT; i++) map->node_index[i] = RIG_VRM_NO_NODE;
    return 0;
}
RIGCOM_PUBLIC int rig_vrm_bonemap_set__rig_dup_fb963a53(RigVRMBoneMap *map, RigVRMBoneIndex bone, int gltf_node_index) {
...
RIGCOM_PUBLIC int rig_vrm_bonemap_validate__rig_dup_d866b03a(const RigVRMBoneMap *map, const char **out_missing_names, int max_missing) {
    if (!map) return -1;
    int missing_count = 0;
 ...
 * ---------------------------------------------------------------------- */
static int rig_vrm_emit_expression_preset__rig_dup_c4cd3870(char *buf, int max_len, int written,
...
RIGCOM_PUBLIC int rig_vrm_generate_extension_json__rig_dup_574d9198(const RigVRMBoneMap *map,
                                                   const RigVRMExpressionState *expr,
                                                   int mesh_node_index,
                                                   char *out_json, int max_len) {
    if (!map || !expr || !out_json || max_len <= 0) return -1;
    const char *missing[8];
    int missing_count = rig_vrm_bonemap_validate__rig_dup_d866b03a(map, missing, 8);
    ...
    for (int i = 0; i < preset_count; i++) {
        ...
        w = rig_vrm_emit_expression_preset__rig_dup_c4cd3870(out_json, max_len, w, presets[i].name, presets[i].weight,
...
RIGCOM_PUBLIC int rig_vrm_bonemap_save__rig_dup_d45a39f0(const RigVRMBoneMap *map, FILE *fp) {
    if (!map || !fp) return -1;
...
RIGCOM_PUBLIC int rig_vrm_bonemap_load__rig_dup_0d8c17ff(RigVRMBoneMap *map, FILE *fp) {
    if (!map || !fp) return -1;
