 ...
 * PRNG local
 * ---------------------------------------------------------------------- */
static unsigned int rig_hair_xorshift32__rig_dup_3902baca(unsigned int *state) {
    unsigned int x = *state ? *state : 0x1234567u;
    x ^= x << 13; x ^= x >> 17; x ^= x << 5;
    *state = x;
    return x;
}
static float rig_hair_rand01__rig_dup_e655cd30(unsigned int *state) {
    return (float)(rig_hair_xorshift32__rig_dup_3902baca(state) & 0x00FFFFFFu) / (float)0x01000000u;
}
static float rig_hair_randn__rig_dup_dc79d76a(unsigned int *state) {
    float u1 = rig_hair_rand01__rig_dup_e655cd30(state); if (u1 < 1e-7f) u1 = 1e-7f;
    float u2 = rig_hair_rand01__rig_dup_e655cd30(state);
    return sqrtf(-2.0f * logf(u1)) * cosf(6.283185307f * u2);
}
static float rig_hair_clamp01__rig_dup_f32c8dec(float v) { return v < 0.0f ? 0.0f : (v > 1.0f ? 1.0f : v); }
/* -------------------------------------------------------------------------
 * Inicializacion
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_hair_growth_state_init__rig_dup_c0aae764(RigHairGrowthState *st, unsigned int seed) {
    if (!st) return -1;
    memset(st, 0, sizeof(*st));
    unsigned int rng = seed ? seed : 0xC0FFEEu;
    st->age_years = 25.0f;
    st->graying_onset_age = 28.0f + rig_hair_rand01__rig_dup_e655cd30(&rng) * 20.0f; /* 28-48, variabilidad genetica */
    st->graying_rate = 0.06f + rig_hair_rand01__rig_dup_e655cd30(&rng) * 0.06f;      /* 0.06 - 0.12 /anio */
    st->graying_temple_bias = 0.6f + rig_hair_rand01__rig_dup_e655cd30(&rng) * 0.3f;
    return 0;
}
RIGCOM_PUBLIC int rig_hair_follicle_add__rig_dup_08bcb100(RigHairGrowthState *st, RigHairRegion region,
    ...
    float roll = rig_hair_rand01__rig_dup_e655cd30(&g->rng_state);
    ...
    if (point < prof->anagen_days_mean) {
        ...
        g->stage_duration_days = prof->anagen_days_mean + rig_hair_randn__rig_dup_dc79d76a(&g->rng_state) * prof->anagen_days_stddev;
 ...
 * ---------------------------------------------------------------------- */
static void rig_hair_advance_one__rig_dup_d5db8541(RigFollicleGroup *g, const RigHairRegionProfile *prof, float dt) {
    if (!g->active) return;
    g->stage_age_days += dt;
    if (g->stage == RIG_FOLLICLE_ANAGEN) {
        ...
        if (g->stage_age_days >= g->stage_duration_days) {
            ...
            g->stage_age_days = 0.0f;
            g->stage_duration_days = 3.0f + rig_hair_rand01__rig_dup_e655cd30(&g->rng_state) * 7.0f; /* caida: dias */
        }
    } else { /* EXOGEN */
        if (g->stage_age_days >= g->stage_duration_days) {
            ...
            g->stage_duration_days = prof->anagen_days_mean +
                rig_hair_randn__rig_dup_dc79d76a(&g->rng_state) * prof->anagen_days_stddev;
            if (g->stage_duration_days < 10.0f) g->stage_duration_days = 10.0f;
        }
    }
}
RIGCOM_PUBLIC int rig_hair_growth_update__rig_dup_d4a77d66(RigHairGrowthState *st, float dt_days) {
    if (!st || dt_days <= 0.0f) return -1;
    st->age_years += dt_days / 365.25f;
    for (int i = 0; i < st->group_count; i++) {
        RigFollicleGroup *g = &st->groups[i];
        if (!g->active) continue;
        rig_hair_advance_one__rig_dup_d5db8541(g, &RIG_HAIR_REGION_PROFILES[g->region], dt_days);
    }
    return 0;
}
/* -------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC float rig_hair_graying_fraction__rig_dup_686de268(const RigHairGrowthState *st, const RigFollicleGroup *g) {
    if (!st || !g) return 0.0f;
    if (st->age_years <= st->graying_onset_age) return 0.0f;
    float years_since_onset = st->age_years - st->graying_onset_age;
    float base_progress = rig_hair_clamp01__rig_dup_f32c8dec(years_since_onset * st->graying_rate);
    ...
    float temple_proximity = rig_hair_clamp01__rig_dup_f32c8dec(1.0f - temple_dist / 0.5f);
    ...
    return rig_hair_clamp01__rig_dup_f32c8dec(local_advance);
}
/* -------------------------------------------------------------------------
...
RIGCOM_PUBLIC int rig_hair_growth_generate_shader__rig_dup_8e383170(char *out_glsl, int max_len) {
...
RIGCOM_PUBLIC int rig_hair_growth_save__rig_dup_b2c1be4a(const RigHairGrowthState *st, FILE *fp) {
    if (!st || !fp) return -1;
...
RIGCOM_PUBLIC int rig_hair_growth_load__rig_dup_e59f5f34(RigHairGrowthState *st, FILE *fp) {
    if (!st || !fp) return -1;
