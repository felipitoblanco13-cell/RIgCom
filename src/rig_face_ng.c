...
/* ================================================================
 * FUENTE ABSORBIDA: 16_FACE_NG/src/rig_face_ng_skin.c
 * ================================================================ */
#line 1 "16_FACE_NG/src/rig_face_ng_skin.c"
/*
...
static const char *NG_GLSL_PBR_SSS =
    ...
    "    return clamp__rig_dup_abe31d6a(F0 + (1.0 - F0)*x5, vec3(0.), vec3(1.));\n"
    "}\n\n"
    ...
    "    return clamp__rig_dup_abe31d6a((x*(a*x+b))/(x*(c*x+d)+e), 0.0, 1.0);\n"
    "}\n\n"
    ...
    "    return clamp__rig_dup_abe31d6a(c, 0.0, 1.0);\n"
    "}\n\n"
...
static const char *NG_SKIN_FRAG_MAIN =
    ...
    "    roughness = clamp__rig_dup_abe31d6a(roughness, 0.02, 0.98);\n"
    ...
    "    roughness = clamp__rig_dup_abe31d6a(roughness + sw.roughness_mod, 0.01, 1.0);\n"
    "    /* Maquillaje */\n"
    ...
    "    color  = pow(clamp__rig_dup_abe31d6a(color, 0.0, 1.0), vec3(1.0 / 2.2));\n"
    "    fragColor = vec4(color, 1.0);\n"
    "}\n";

/* ═══════════════════════════════════════════════════════════════
 * rig_face_ng_skin_shader__rig_variant_90dde9bb() — Función principal de generación
 * ═══════════════════════════════════════════════════════════════ */
int rig_face_ng_skin_shader__rig_variant_90dde9bb(const RigFaceNGSkinCtx *ctx, RigArtResultNG *out)
{
    if (!ctx || !out) return -1;

    char *frag = (char*)malloc(SKIN_BUF);
    char *vert = (char*)malloc(SKIN_BUF / 2);
    char *js   = (char*)malloc(JS_BUF);
    if (!frag || !vert || !js) {
        free(frag); free(vert); free(js);
        return -1;
    }
...
/* ================================================================
 * FUENTE ABSORBIDA: 16_FACE_NG/src/rig_face_ng_eyes.c
 * ================================================================ */
#line 1 "16_FACE_NG/src/rig_face_ng_eyes.c"
/*
...
static const char *NG_EYE_LIB_GLSL =
    ...
    "    }\n"
    "    vessel_pattern = clamp__rig_dup_abe31d6a(vessel_pattern * hemoglobin, 0., 1.);\n"
    ...
    "    r.color       = clamp__rig_dup_abe31d6a(sclera_base, 0., 1.);\n"
...
static const char *NG_EYE_FRAG_MAIN =
    ...
    "    /* ── Tone mapping ── */\n"
    "    color *= u_exposure;\n"
    "    color  = clamp__rig_dup_abe31d6a((color*(2.51*color+0.03))/(color*(2.43*color+0.59)+0.14), 0., 1.);\n"
...
static void eye_dynamics_init__rig_variant_ec68667d(EyeDynamicsNG *e, float light_level)
{
    if (!e) return;
    memset(e, 0, sizeof(*e));
...
void eye_dynamics_update__rig_variant_1a3c239e(EyeDynamicsNG *e, float dt)
{
    if (!e || dt <= 0.0f) return;
    e->time_acc  += dt;
    e->micro_time+= dt;
    ...
    e->pupil_target = clamp__rig_dup_abe31d6a(baseline_pupil + hipus, 1.5f, 8.5f);
...
void eye_dynamics_set_gaze__rig_variant_00e39fb8(EyeDynamicsNG *e, float h, float v)
{
    if (!e) return;
    float dist = sqrtf((h-e->gaze_h)*(h-e->gaze_h) + (v-e->gaze_v)*(v-e->gaze_v));
    e->gaze_target_h = clamp__rig_dup_abe31d6a(h, -30.0f, 30.0f);
    e->gaze_target_v = clamp__rig_dup_abe31d6a(v, -20.0f, 20.0f);
 ...
 * ═══════════════════════════════════════════════════════════════ */
int rig_face_ng_eye_shader__rig_variant_2e260dfd(const RigEyeNGCtx *ctx, RigArtResultNG *out)
{
    if (!ctx || !out) return -1;
    char *frag = (char*)malloc(EYE_BUF);
    char *js   = (char*)malloc(EYE_JS_BUF);
...
/* ================================================================
 * FUENTE ABSORBIDA: 16_FACE_NG/src/rig_face_ng_hair.c
 * ================================================================ */
#line 1 "16_FACE_NG/src/rig_face_ng_hair.c"
/*
...
static const char *NG_HAIR_MARSCHNER_GLSL =
    ...
    "    F = clamp__rig_dup_abe31d6a(F, 0.0, 1.0);\n"
    ...
    "    float phi = acos(clamp__rig_dup_abe31d6a(cos_phi, -1., 1.));\n"
...
static const char *NG_HAIR_FRAG_GLSL =
    ...
    "float h11(float p) { p=fract(p*.1031); p*=p+33.33; return fract(p*(p+p)); }\n\n"
    "vec3 ACES(vec3 x) {\n"
    "    return clamp__rig_dup_abe31d6a((x*(2.51*x+0.03))/(x*(2.43*x+0.59)+0.14),0.,1.);\n"
    "}\n\n"
    "void main() {\n"
    "    vec3 V = normalize(u_view_dir);\n"
    ...
    "    float mel_eu = clamp__rig_dup_abe31d6a(v_mel_eu + var * 0.3, 0.0, 1.0);\n"
    "    float mel_ph = clamp__rig_dup_abe31d6a(v_mel_ph + var * 0.1, 0.0, 1.0);\n\n"
    ...
    "    color *= u_exposure;\n"
    "    color  = ACES(color);\n"
    "    color  = pow(clamp__rig_dup_abe31d6a(color,0.,1.), vec3(1./2.2));\n"
...
static void hair_strand_init__rig_variant_bc586253(
    HairStrand *strand,
...
void hair_sim_step__rig_variant_bc304ddf(HairSimSystem *sys, float dt)
{
...
void hair_sim_free__rig_variant_8d15826e(HairSimSystem *sys)
{
    if (!sys) return;
 ...
 * ═══════════════════════════════════════════════════════════════ */
int rig_face_ng_hair_shader__rig_variant_926849a2(const RigHairNGCtx *ctx, RigArtResultNG *out)
{
    if (!ctx || !out) return -1;
...
/* ================================================================
 * FUENTE ABSORBIDA: 16_FACE_NG/src/rig_face_ng_anim.c
 * ================================================================ */
#line 1 "16_FACE_NG/src/rig_face_ng_anim.c"
/*
...
/* ════════════════════════════════════════════════════════════════
 * rig_face_ng_anim_generate__rig_variant_ccc38cc1() — Función principal de generación
 * ════════════════════════════════════════════════════════════════ */
int rig_face_ng_anim_generate__rig_variant_ccc38cc1(const RigFaceNGAnimCtx *ctx, RigArtResultNG *out)
{
    if (!ctx || !out) return -1;

    char *js   = (char*)malloc(ANIM_BUF);
    char *vert = (char*)malloc(ANIM_BUF / 2);
    if (!js || !vert) { free(js); free(vert); return -1; }
    int jp=0, vp=0;
    ...
    /* ── Runtime JS ── */
    for (size_t i = 0; i < sizeof(NG_ANIM_JS_RUNTIME)/sizeof(NG_ANIM_JS_RUNTIME[0]); i++)
        FA(js, jsz, jp, "%s", NG_ANIM_JS_RUNTIME[i]);

    /* ── Vertex Shader ── */
    FA(vert, vsz, vp, "// §NG-ANIM Vertex Shader — RigCom NG\n");
    for (size_t i = 0; i < sizeof(NG_ANIM_GLSL_VERT)/sizeof(NG_ANIM_GLSL_VERT[0]); i++)
        FA(vert, vsz, vp, "%s", NG_ANIM_GLSL_VERT[i]);
...
/* ================================================================
 * FUENTE ABSORBIDA: 16_FACE_NG/src/rig_face_ng_assembly.c
 * ================================================================ */
#line 1 "16_FACE_NG/src/rig_face_ng_assembly.c"
/*
...
"  col=pow(clamp__rig_dup_abe31d6a(col,0.,1.),vec3(1./2.2));\n"
"  fc=vec4(col,1.);\n"
"}`;\n"
...
/* ════════════════════════════════════════════════════════════════
 * rig_face_ng_generate_html__rig_variant_a4edbd06() — Genera el HTML standalone completo
 * ════════════════════════════════════════════════════════════════ */
int rig_face_ng_generate_html__rig_variant_a4edbd06(const RigFaceNGParams *params,
...
/* ════════════════════════════════════════════════════════════════
 * rig_face_ng_assemble__rig_variant_08d406e3() — Función principal de ensamblaje
 * ════════════════════════════════════════════════════════════════ */
int rig_face_ng_assemble__rig_variant_08d406e3(const RigFaceNGParams *params,
                          const RigFaceNGRenderCtx *render,
                          RigArtResultNG *out)
{
    ...
    int len = rig_face_ng_generate_html__rig_variant_a4edbd06(params, render, html, HTML_BUF);
...
/* ════════════════════════════════════════════════════════════════
 * rig_face_ng_params_default__rig_variant_c29d5d89() — Defaults de producción
 * ════════════════════════════════════════════════════════════════ */
void rig_face_ng_params_default__rig_variant_c29d5d89(RigFaceNGParams *p)
{
    if (!p) return;
    memset(p, 0, sizeof(*p));
...
void rig_face_ng_result_free__rig_variant_a9c84603(RigArtResultNG *r)
{
    if (!r) return;
