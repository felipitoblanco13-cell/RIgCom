...
static float  b4_f__rig_variant_7f34e6c8(const char *j, const char *k, float  d)
{
    if (!j) return d;
...
static int    b4_i(const char *j, const char *k, int    d)
{
    ...
    if (!p) return d;
    p = rl_strchr(p, ':');
    return p ? (int)rl_atoi(p+1) : d;
}
static int    b4_b__rig_variant_924c7242(const char *j, const char *k, int    d)
{
    if (!j) return d;
    const char *p = rl_strstr(j, k);
    if (!p) return d;
...
int rigart_face_session_v2_init_BR(RIgArtFaceSessionV2 *s)
{
    if (!s) return 0;
...
int rigart_face_session_v2_destroy_BR(RIgArtFaceSessionV2 *s)
{
    if (!s) return 0;
...
int rigart_face_v2_dispatch_BR(WsServer *srv, const char *cmd,
                              const char *payload,
                              RIgArtFaceSessionV2 *session)
{
    ...
    {
        session->params.melanin          = b4_f__rig_variant_7f34e6c8(payload, "\"melanin\"",    0.35f);
        session->params.hemoglobin       = b4_f__rig_variant_7f34e6c8(payload, "\"hemoglobin\"", 0.40f);
        session->params.age_factor       = b4_f__rig_variant_7f34e6c8(payload, "\"age\"",        0.25f);
        session->params.gender_factor    = b4_f__rig_variant_7f34e6c8(payload, "\"gender\"",     0.50f);
        session->params.cranium_width    = b4_f__rig_variant_7f34e6c8(payload, "\"cw\"",  session->params.cranium_width);
        session->params.cranium_height   = b4_f__rig_variant_7f34e6c8(payload, "\"ch\"",  session->params.cranium_height);
        session->params.cranium_depth    = b4_f__rig_variant_7f34e6c8(payload, "\"cd\"",  session->params.cranium_depth);
        session->params.jaw_width        = b4_f__rig_variant_7f34e6c8(payload, "\"jaw_w\"",session->params.jaw_width);
        session->params.jaw_angle        = b4_f__rig_variant_7f34e6c8(payload, "\"jaw_a\"",session->params.jaw_angle);
        session->params.nose_length      = b4_f__rig_variant_7f34e6c8(payload, "\"nose_l\"",session->params.nose_length);
        session->params.nose_width       = b4_f__rig_variant_7f34e6c8(payload, "\"nose_w\"",session->params.nose_width);
        session->params.interocular_dist = b4_f__rig_variant_7f34e6c8(payload, "\"iod\"",  session->params.interocular_dist);
        session->params.zygomatic_width  = b4_f__rig_variant_7f34e6c8(payload, "\"zygo_w\"",session->params.zygomatic_width);
        session->params.mouth_width      = b4_f__rig_variant_7f34e6c8(payload, "\"mouth_w\"",session->params.mouth_width);
        session->params.neck_width       = b4_f__rig_variant_7f34e6c8(payload, "\"neck_w\"",session->params.neck_width);
        session->params.neck_length      = b4_f__rig_variant_7f34e6c8(payload, "\"neck_l\"",session->params.neck_length);
        session->params.chin_projection  = b4_f__rig_variant_7f34e6c8(payload, "\"chin_p\"",session->params.chin_projection);
        ...
        bool build_pores    = (bool)b4_b__rig_variant_924c7242(payload, "\"pores\"",    1);
        bool build_vascular = (bool)b4_b__rig_variant_924c7242(payload, "\"vascular\"", 1);
        bool build_wrinkles = (bool)b4_b__rig_variant_924c7242(payload, "\"wrinkles\"", 1);
        float age_for_wrinkles = b4_f__rig_variant_7f34e6c8(payload, "\"age\"", 0.25f);
    ...
    if (rl_strcmp(cmd, "rigart_face_archetype") == 0)
    {
    ...
    if (rl_strcmp(cmd, "rigart_face_age") == 0)
    {
        if (!session->mesh) {
            rl_snprintf(resp, sizeof(resp),
        ...
        float age = b4_f__rig_variant_7f34e6c8(payload, "\"age\"", 25.0f);
    ...
    if (rl_strcmp(cmd, "rigart_face_codegen") == 0)
    {
        if (!session->mesh) {
            rl_snprintf(resp, sizeof(resp),
        ...
        rl_memset(&opt, 0, sizeof(opt));
        opt.include_comments  = true;
        opt.arm64_intrinsics  = true;
        opt.include_mesh_data = true;

        if      (rl_strcmp(target_str, "rigscript") == 0) opt.target = RIG_CODEGEN_RIGSCRIPT;
        else if (rl_strcmp(target_str, "json")       == 0) opt.target = RIG_CODEGEN_JSON;
        else if (rl_strcmp(target_str, "glsl")       == 0) opt.target = RIG_CODEGEN_GLSL;
...
int rigart_renderer_html_v4_BR(const RIgArtRendererCtxV4 *ctx, RigArtResultV4 *out)
{
    ...
    rl_snprintf(canvas.layers[0].name, 64, "RigArt v4 — %s",
    ...
    int pos = rl_snprintf(html, BRIDGE_HTML_CAP,
"<!DOCTYPE html>\n<html lang=\"es\">\n<head>\n"
"<meta charset=\"UTF-8\">\n"
