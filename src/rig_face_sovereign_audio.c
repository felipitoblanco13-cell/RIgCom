...
static float clampf__rig_dup_53f7ba2c(float x,float a,float b){return x<a?a:(x>b?b:x);}
...
static rl_size next_pow2(rl_size n){rl_size p=1;while(p<n&&p<(1u<<20))p<<=1;return p;}
static void fft(float *re,float *im,rl_size n){rl_size i,j;for(i=1,j=0;i<n;i++){rl_size b=n>>1;for(;j&b;b>>=1)j^=b;j^=b;if(i<j){float t=re[i];re[i]=re[j];re[j]=t;t=im[i];im[i]=im[j];im[j]=t;}}for(rl_size len=2;len<=n;len<<=1){float ang=(float)(-2.0*M_PI/len),wr0=cosf(ang),wi0=sinf(ang);for(i=0;i<n;i+=len){float wr=1,wi=0;for(j=0;j<len/2;j++){rl_size u=i+j,v=i+j+len/2;float tr=wr*re[v]-wi*im[v],ti=wr*im[v]+wi*re[v],ur=re[u],ui=im[u];re[u]=ur+tr;im[u]=ui+ti;re[v]=ur-tr;im[v]=ui-ti;{float nr=wr*wr0-wi*wi0;wi=wr*wi0+wi*wr0;wr=nr;}}}}}

typedef struct {float energy,zcr,flat,centroid,pitch,f1,f2,voiced;} Features;
static Features features(const float *s,rl_size n,unsigned rate,float *re,float *im,rl_size N){Features f={0};rl_size i,k;double e=0,z=0,gm=0,am=0,cs=0,sw=0;float prev=s[0];for(i=0;i<n;i++){float w=0.54f-0.46f*cosf((float)(2*M_PI*i/(n-1)));float x=s[i]*w;e+=x*x;if(i&&((s[i]>=0)!=(prev>=0)))z++;prev=s[i];re[i]=x;im[i]=0;}for(i=n;i<N;i++)re[i]=im[i]=0;f.energy=sqrtf((float)(e/n));f.zcr=(float)(z/n);fft(re,im,N);for(k=1;k<N/2;k++){float mag=sqrtf(re[k]*re[k]+im[k]*im[k])+1e-12f;float hz=(float)k*rate/N;am+=mag;gm+=log(mag);cs+=hz*mag;sw+=mag;}f.centroid=sw?(float)(cs/sw):0;f.flat=am?(float)(exp(gm/(N/2-1))/(am/(N/2-1))):0;
    /* autocorrelation pitch */{rl_size minlag=rate/450,maxlag=rate/65,lag,bestlag=0;double best=0,zero=0;if(maxlag>=n)maxlag=n-1;for(i=0;i<n;i++)zero+=s[i]*s[i];for(lag=minlag;lag<=maxlag;lag++){double c=0;for(i=0;i+lag<n;i++)c+=s[i]*s[i+lag];if(c>best){best=c;bestlag=lag;}}f.voiced=zero>1e-9?(float)(best/zero):0;f.pitch=bestlag?(float)rate/bestlag:0;}
    /* two strongest separated low-frequency peaks approximate F1/F2 */{float best1=0,best2=0;rl_size k1=0,k2=0;rl_size maxk=(rl_size)(4000.0*N/rate);if(maxk>N/2)maxk=N/2;for(k=2;k+2<maxk;k++){float m=re[k]*re[k]+im[k]*im[k];if(m>re[k-1]*re[k-1]+im[k-1]*im[k-1]&&m>re[k+1]*re[k+1]+im[k+1]*im[k+1]){float hz=(float)k*rate/N;if(hz>180&&hz<1200&&m>best1){best1=m;k1=k;}else if(hz>700&&m>best2){best2=m;k2=k;}}}f.f1=(float)k1*rate/N;f.f2=(float)k2*rate/N;if(f.f2<f.f1+300)f.f2=f.centroid;}
    return f;}
...
static int track_push(RigSovPhonemeTrack *t,const RigSovPhonemeEvent *e){if(t->count==t->capacity){rl_size nc=t->capacity?t->capacity*2:128;RigSovPhonemeEvent *ne=(RigSovPhonemeEvent*)rl_realloc(t->events,nc*sizeof(*ne));if(!ne)return RIG_SOV_ENOMEM;t->events=ne;t->capacity=nc;}t->events[t->count++]=*e;return RIG_SOV_OK;}

int rig_sov_audio_to_phonemes(const RigSovAudio *a,RigSovPhonemeTrack *t){rl_size frame,hop,N,pos;float *re=NULL,*im=NULL,prevE=0;int current=-1,rc=0;RigSovPhonemeEvent ev={0};if(!a||!t||!a->samples||a->sample_rate<8000||a->sample_count<a->sample_rate/20)return RIG_SOV_EINVAL;rl_memset(t,0,sizeof(*t));t->sample_rate=a->sample_rate;frame=(rl_size)(a->sample_rate*0.025f);hop=(rl_size)(a->sample_rate*0.010f);if(frame<128)frame=128;if(hop<32)hop=32;N=next_pow2(frame);re=(float*)rl_malloc(N*sizeof(float));im=(float*)rl_malloc(N*sizeof(float));if(!re||!im){rl_free(re);rl_free(im);return RIG_SOV_ENOMEM;}for(pos=0;pos+frame<=a->sample_count;pos+=hop){Features f=features(a->samples+pos,frame,a->sample_rate,re,im,N);int id=classify(&f,prevE);float start=(float)pos/a->sample_rate,end=(float)(pos+frame)/a->sample_rate,conf=clampf__rig_dup_53f7ba2c(0.35f+0.45f*f.voiced+0.25f*(1-f.flat),0.1f,1.0f);if(id!=current){if(current>=0){ev.end_s=start;if(ev.end_s-ev.start_s>=0.008f){rc=track_push(t,&ev);if(rc)break;}}current=id;ev.id=id;ev.start_s=start;ev.end_s=end;ev.confidence=conf;ev.jaw_open=PH[id].jaw;ev.lip_rounding=PH[id].round;ev.lip_spreading=PH[id].spread;ev.lip_protrusion=PH[id].protrude;}else{ev.end_s=end;ev.confidence=(ev.confidence+conf)*0.5f;}prevE=f.energy;}if(!rc&&current>=0)rc=track_push(t,&ev);rl_free(re);rl_free(im);if(rc){rig_sov_phoneme_track_free(t);return rc;}return t->count?RIG_SOV_OK:RIG_SOV_EFORMAT;}

int rig_sov_phoneme_track_write_csv(const RigSovPhonemeTrack *t,const char *path){FILE *f;rl_size i;if(!t||!path||!t->events)return RIG_SOV_EINVAL;f=fopen(path,"wb");if(!f)return RIG_SOV_EIO;fprintf(f,"id,ipa,start_s,end_s,confidence,jaw_open,lip_rounding,lip_spreading,lip_protrusion\n");for(i=0;i<t->count;i++){const RigSovPhonemeEvent *e=&t->events[i];fprintf(f,"%d,\"%s\",%.6f,%.6f,%.6f,%.6f,%.6f,%.6f,%.6f\n",e->id,rig_sov_phoneme_ipa(e->id),e->start_s,e->end_s,e->confidence,e->jaw_open,e->lip_rounding,e->lip_spreading,e->lip_protrusion);}if(fclose(f)!=0)return RIG_SOV_EIO;return RIG_SOV_OK;}
void rig_sov_phoneme_track_free(RigSovPhonemeTrack *t){if(t){rl_free(t->events);rl_memset(t,0,sizeof(*t));}}
