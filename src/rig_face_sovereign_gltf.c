...
typedef struct { rl_u8 *p; rl_size n,cap; } Blob;
typedef struct { char *p; rl_size n,cap; } Text;

static int blob_reserve(Blob *b,rl_size add){rl_size need=b->n+add;if(need<b->n)return RIG_SOV_ERANGE;if(need>b->cap){rl_size nc=b->cap?b->cap:4096;while(nc<need){if(nc>SIZE_MAX/2)return RIG_SOV_ERANGE;nc*=2;}rl_u8 *np=(rl_u8*)rl_realloc(b->p,nc);if(!np)return RIG_SOV_ENOMEM;b->p=np;b->cap=nc;}return RIG_SOV_OK;}
static int blob_add(Blob *b,const void *p,rl_size n){int rc=blob_reserve(b,n);if(rc)return rc;rl_memcpy(b->p+b->n,p,n);b->n+=n;return RIG_SOV_OK;}
static int blob_zero_align(Blob *b,rl_size a){rl_u8 z=0;while(b->n%a){int rc=blob_add(b,&z,1);if(rc)return rc;}return RIG_SOV_OK;}
static int text_reserve(Text *t,rl_size add){rl_size need=t->n+add+1;if(need<t->n)return RIG_SOV_ERANGE;if(need>t->cap){rl_size nc=t->cap?t->cap:4096;while(nc<need){if(nc>SIZE_MAX/2)return RIG_SOV_ERANGE;nc*=2;}char *np=(char*)rl_realloc(t->p,nc);if(!np)return RIG_SOV_ENOMEM;t->p=np;t->cap=nc;}return RIG_SOV_OK;}
static int textf(Text *t,const char *fmt,...){va_list ap,cp;int n,rc;va_start(ap,fmt);va_copy(cp,ap);n=vsnprintf(NULL,0,fmt,cp);va_end(cp);if(n<0){va_end(ap);return RIG_SOV_EIO;}rc=text_reserve(t,(rl_size)n);if(rc){va_end(ap);return rc;}vsnprintf(t->p+t->n,t->cap-t->n,fmt,ap);va_end(ap);t->n+=(rl_size)n;return RIG_SOV_OK;}
...
static void wr32(FILE *f,rl_u32 v){rl_u8 b[4]={(rl_u8)v,(rl_u8)(v>>8),(rl_u8)(v>>16),(rl_u8)(v>>24)};fwrite(b,1,4,f);}

int rig_sov_export_glb(const RigSovMesh *m,const RigSovSkeleton *s,const RigSovMaterial *mat,const char *path){
    Blob bin={0};Text js={0};rl_size off[7]={0},len[7]={0},i,j;float minp[3],maxp[3];int has_skin=s&&s->joint_count>0;int rc;FILE *f=NULL;rl_u32 json_len,bin_len,total;RigSovMaterial def={{0.55f,0.32f,0.24f},0.45f,0,0};
...
#define ... (do{off[ID]=bin.n;for(i=0;i<m->vertex_count;i++){TYPE v=(EXPR);if((rc=blob_add(&bin,&v,sizeof(v))))goto done;}len[ID]=bin.n-off[ID];if((rc=blob_zero_align(&bin,4)))goto done;}while(0))
    ...
    if(has_skin){typedef struct{rl_u16 x,y,z,w;} U4;ADD_SECTION(3,U4,((U4){m->vertices[i].joints[0],m->vertices[i].joints[1],m->vertices[i].joints[2],m->vertices[i].joints[3]}));}
...
#undef ADD_SECTION
    off[5]=bin.n;for(i=0;i<m->triangle_count;i++){rl_u32 idx[3]={m->triangles[i].a,m->triangles[i].b,m->triangles[i].c};if((rc=blob_add(&bin,idx,sizeof(idx))))goto done;}len[5]=bin.n-off[5];if((rc=blob_zero_align(&bin,4)))goto done;
    ...
    if(js.n>UINT32_MAX||bin.n>UINT32_MAX||12u+8u+js.n+8u+bin.n>UINT32_MAX){rc=RIG_SOV_ERANGE;goto done;}json_len=(rl_u32)js.n;bin_len=(rl_u32)bin.n;total=12+8+json_len+8+bin_len;
...
done:if(f&&fclose(f)!=0&&rc==RIG_SOV_OK)rc=RIG_SOV_EIO;rl_free(bin.p);rl_free(js.p);return rc;
}
