/* ==========================================================================
 * 21_rig_face_codegen.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : nested/Face_Pose_Body/rig_face_codegen.c
 * Copias fundidas: 7
 * Funciones      : 9      Unidades injertadas: 2      Variantes: 2
 *
...
static int cg_timestamp(char *buf, size_t sz)
{
    time_t t = time(NULL);
...
/* RIGCOM_PUBLIC_STATIC_EXPORT_BEGIN: cg_timestamp -> rigpub_rig_face_codegen_cg_timestamp */
int (*rigpub_rig_face_codegen_cg_timestamp)(char *buf, size_t sz) = cg_timestamp;
/* RIGCOM_PUBLIC_STATIC_EXPORT_END */


int rig_codegen_params(const RigFaceParams *p,
                       ...
                       char *buf, size_t sz)
{
    ...
    if (cmt) {
        char ts[32]; cg_timestamp(ts, sizeof(ts));
...
int rig_codegen_rigscript_face(const RigFaceArchetype *arch, char *buf, size_t sz)
{
...
int rig_codegen_archetype(RigArchetypeID id,
                           const RigCodegenOptions *opt,
                           char *buf, size_t sz)
{
    ...
    if (opt && opt->target == RIG_CODEGEN_RIGSCRIPT) {
        return rig_codegen_rigscript_face(arch, buf, sz);
    }

    if (opt && opt->target == RIG_CODEGEN_HEADER) {
        return rig_codegen_all_archetypes_header(buf, sz);
    }
...
int rig_codegen_glsl_skin_shader(const RigFaceMeshV2 *mesh, char *buf, size_t sz)
{
...
int rig_codegen_all_archetypes_header(char *buf, size_t sz)
{
...
int rig_codegen_makefile(const char *project_name, char *buf, size_t sz)
{
...
int rig_codegen_mesh_v2(const RigFaceMeshV2 *mesh,
                         const RigCodegenOptions *opt,
                         char *buf, size_t sz)
{
    if (!mesh || !buf || sz == 0) return -1;
    ...
    return rig_codegen_params(&mesh->base.params, opt, buf, sz);
}
...
/* [fusion] global sz__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_codegen.c:24 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_codegen_cg_timestamp)(char *buf, size_t sz__v2) = cg_timestamp;

/* [fusion] function cg_timestamp__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_codegen.c:14 :: cuerpo divergente; ausente en la copia canonica */
static void cg_timestamp__v2(char *buf, size_t sz)
{
    time_t t = time(NULL);
    struct tm *tm_info = gmtime(&t);
    if (tm_info)
