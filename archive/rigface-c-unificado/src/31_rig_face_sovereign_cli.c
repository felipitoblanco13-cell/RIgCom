/* ==========================================================================
 * 31_rig_face_sovereign_cli.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : nested/Face_Pose_Body/rig_face_sovereign_cli.c
 * Copias fundidas: 5
...
static void join_path(char *out,size_t n,const char *dir,const char *name){size_t l=strlen(dir);snprintf(out,n,"%s%s%s",dir,(l&&dir[l-1]=='/')?"":"/",name);}
static void pixel(RigSovImage *im,int x,int y,uint8_t r,uint8_t g,uint8_t b){size_t p;if(x<0||y<0||x>=(int)im->width||y>=(int)im->height)return;p=((size_t)y*im->width+x)*3;im->pixels[p]=r;im->pixels[p+1]=g;im->pixels[p+2]=b;}
static void ellipse(RigSovImage *im,float cx,float cy,float rx,float ry,uint8_t r,uint8_t g,uint8_t b){int x,y;for(y=(int)(cy-ry);y<=(int)(cy+ry);y++)for(x=(int)(cx-rx);x<=(int)(cx+rx);x++){float dx=(x-cx)/rx,dy=(y-cy)/ry;if(dx*dx+dy*dy<=1)pixel(im,x,y,r,g,b);}}
static int synthetic_face(RigSovImage *im){int x,y;memset(im,0,sizeof(*im));im->width=256;im->height=256;im->channels=3;im->pixels=(uint8_t*)malloc((size_t)256*256*3);if(!im->pixels)return RIG_SOV_ENOMEM;for(y=0;y<256;y++)for(x=0;x<256;x++)pixel(im,x,y,34,48,65);ellipse(im,128,130,72,96,188,126,94);ellipse(im,128,112,17,31,205,143,105);ellipse(im,98,113,17,8,238,238,224);ellipse(im,158,113,17,8,238,238,224);ellipse(im,98,113,5,5,45,28,20);ellipse(im,158,113,5,5,45,28,20);ellipse(im,128,163,28,8,116,38,45);ellipse(im,128,160,24,3,221,114,112);for(x=106;x<=122;x++)pixel(im,x,93,65,40,31);for(x=134;x<=150;x++)pixel(im,x,93,65,40,31);return RIG_SOV_OK;}

int rig_sov_selftest(const char *dir, char *report, size_t report_size) {
    ...
    size_t body_vertices = 0, body_triangles = 0, i;
    int rc = RIG_SOV_OK;
...
int main(int argc,char **argv){int rc=RIG_SOV_EINVAL;if(argc<2){usage(argv[0]);return 2;}if(!strcmp(argv[1],"selftest")){char report[512];if(argc!=3){usage(argv[0]);return 2;}rc=rig_sov_selftest(argv[2],report,sizeof(report));fprintf(rc?stderr:stdout,"%s\n",report);}
    ...
    else if(!strcmp(argv[1],"camera")){unsigned frames,w=640,h=480;if(argc<5||argc>7){usage(argv[0]);return 2;}frames=(unsigned)atoi(argv[4]);if(argc>5)w=(unsigned)atoi(argv[5]);if(argc>6)h=(unsigned)atoi(argv[6]);rc=rig_sov_camera_track(argv[2],w,h,frames,argv[3]);}
