/* ==========================================================================
 * 16_rig_face_soft_body.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : src_raw/rig_face_soft_body-1.c
 * Copias fundidas: 1
 * Funciones      : 15      Unidades injertadas: 0      Variantes: 0
 *
...
static RigSBVec3 rig_sb_add(RigSBVec3 a, RigSBVec3 b) { RigSBVec3 r = {a.x+b.x,a.y+b.y,a.z+b.z}; return r; }
static RigSBVec3 rig_sb_sub(RigSBVec3 a, RigSBVec3 b) { RigSBVec3 r = {a.x-b.x,a.y-b.y,a.z-b.z}; return r; }
static RigSBVec3 rig_sb_scale(RigSBVec3 a, float s) { RigSBVec3 r = {a.x*s,a.y*s,a.z*s}; return r; }
static float rig_sb_len(RigSBVec3 a) { return sqrtf(a.x*a.x+a.y*a.y+a.z*a.z); }
 ...
 * Gestion de regiones
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_softbody_init(RigSoftBodySystem *sys) {
    if (!sys) return -1;
    memset(sys, 0, sizeof(*sys));
    return 0;
}

RIGCOM_PUBLIC int rig_softbody_region_add(RigSoftBodySystem *sys, const char *name, int bone_index,
...
RIGCOM_PUBLIC int rig_softbody_region_add_preset_chest(RigSoftBodySystem *sys, int bone_index, int is_right) {
    ...
    return rig_softbody_region_add(sys, name, bone_index, anchor, k, c, 0.9f, 35.0f, 90.0f);
}
RIGCOM_PUBLIC int rig_softbody_region_add_preset_glute(RigSoftBodySystem *sys, int bone_index, int is_right) {
    ...
    return rig_softbody_region_add(sys, name, bone_index, anchor, k, c, 1.2f, 30.0f, 110.0f);
}
RIGCOM_PUBLIC int rig_softbody_region_add_preset_belly(RigSoftBodySystem *sys, int bone_index) {
    RigSBVec3 anchor = { 0.0f, -20.0f, 30.0f };
    RigSBVec3 k = { 150.0f, 100.0f, 130.0f };
    RigSBVec3 c = { 13.0f, 10.0f, 12.0f };
    return rig_softbody_region_add(sys, "belly", bone_index, anchor, k, c, 1.0f, 40.0f, 120.0f);
}
RIGCOM_PUBLIC int rig_softbody_region_add_preset_cheek(RigSoftBodySystem *sys, int bone_index, int is_right) {
    ...
    return rig_softbody_region_add(sys, name, bone_index, anchor, k, c, 0.08f, 6.0f, 35.0f);
}
RIGCOM_PUBLIC int rig_softbody_region_add_preset_upper_arm(RigSoftBodySystem *sys, int bone_index, int is_right) {
    ...
    return rig_softbody_region_add(sys, name, bone_index, anchor, k, c, 0.5f, 20.0f, 70.0f);
}
 ...
 * esqueleto existente).
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_softbody_update(RigSoftBodySystem *sys, const RigSBVec3 *bone_world_pos,
    ...
    for (int i = 0; i < sys->region_count; i++) {
        ...
        RigSBVec3 bone_vel = rig_sb_scale(rig_sb_sub(bone_pos, st->prev_bone_world_pos), 1.0f / dt);
        RigSBVec3 bone_accel = rig_sb_scale(rig_sb_sub(bone_vel, st->prev_bone_world_vel), 1.0f / dt);
        ...
        for (int s = 0; s < RIG_JIGGLE_SUBSTEPS; s++) {
            ...
            RigSBVec3 inertial_force = rig_sb_scale(bone_accel, -cfg->mass);

            RigSBVec3 total_force = rig_sb_add(rig_sb_add(spring_force, damping_force), inertial_force);
            RigSBVec3 accel = rig_sb_scale(total_force, 1.0f / cfg->mass);

            /* semi-implicito: actualiza velocidad primero, luego posicion con la nueva velocidad */
            st->velocity_mm_s = rig_sb_add(st->velocity_mm_s, rig_sb_scale(accel, sub_dt));
            st->displacement_mm = rig_sb_add(st->displacement_mm, rig_sb_scale(st->velocity_mm_s, sub_dt));
        }

        /* clamp de seguridad: evita explosion numerica ante aceleraciones extremas */
        float disp_len = rig_sb_len(st->displacement_mm);
        if (disp_len > cfg->max_displacement_mm && disp_len > 1e-6f) {
            float scale = cfg->max_displacement_mm / disp_len;
            st->displacement_mm = rig_sb_scale(st->displacement_mm, scale);
            st->velocity_mm_s = rig_sb_scale(st->velocity_mm_s, 0.7f); /* disipa energia extra en el clamp */
        }
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_softbody_eval_vertex_offset(const RigSoftBodySystem *sys, int region_index,
    ...
    RigSBVec3 delta = rig_sb_sub(vertex_world_pos_mm, region_anchor_world_pos_mm);
    float dist = rig_sb_len(delta);
    float sigma = cfg->influence_radius_mm * 0.5f;
    float weight = expf(-(dist * dist) / (2.0f * sigma * sigma + 1e-6f));

    *out_offset_mm = rig_sb_scale(st->displacement_mm, weight);
    return 0;
}
...
RIGCOM_PUBLIC int rig_softbody_save(const RigSoftBodySystem *sys, FILE *fp) {
    if (!sys || !fp) return -1;
...
RIGCOM_PUBLIC int rig_softbody_load(RigSoftBodySystem *sys, FILE *fp) {
    if (!sys || !fp) return -1;
