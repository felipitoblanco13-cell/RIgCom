/* ==========================================================================
 * 19_rig_face_voice_synth.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : src_raw/rig_face_voice_synth-1.c
 * Copias fundidas: 1
 ...
 * PRNG local
 * ---------------------------------------------------------------------- */
static unsigned int rig_voice_xorshift32(unsigned int *state) {
    unsigned int x = *state ? *state : 0xBADC0FFEu;
    x ^= x << 13; x ^= x >> 17; x ^= x << 5; *state = x; return x;
}
static float rig_voice_rand_bipolar(unsigned int *state) {
    return ((float)(rig_voice_xorshift32(state) & 0x00FFFFFFu) / (float)0x00800000u) - 1.0f;
}

/* -------------------------------------------------------------------------
 * Script API
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_voice_script_init(RigVoiceScript *script) {
    if (!script) return -1;
...
RIGCOM_PUBLIC int rig_voice_script_add_phoneme(RigVoiceScript *script, RigPhonemeId id, float duration_ms_or_zero) {
    if (!script) return -1;
...
RIGCOM_PUBLIC int rig_voice_script_add_pitch_key(RigVoiceScript *script, float time_ms, float f0_hz) {
    if (!script) return -1;
...
static float rig_voice_f0_at(const RigVoiceScript *script, float time_ms) {
...
static float rig_voice_formant_process(RigFormantResonatorState *st, float x,
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_voice_synthesize(const RigVoiceScript *script, float sample_rate,
    ...
    for (int e = 0; e < script->entry_count; e++) {
        ...
        for (int i = 0; i < n_samples; i++) {
            ...
            float f0 = rig_voice_f0_at(script, t_ms);
            float jitter = 1.0f + (script->jitter_percent * 0.01f) * rig_voice_rand_bipolar(&rng);
            ...
            if (cur->is_voiced || cur->noise_mix < 1.0f) {
                ...
                if (phase >= 1.0f) {
                    phase -= 1.0f;
                    float shimmer = 1.0f + (script->shimmer_percent * 0.01f) * rig_voice_rand_bipolar(&rng);
                    pulse = shimmer;
                }
            ...
            if (cur->noise_mix > 0.0f || !cur->is_voiced) {
                float noise = rig_voice_rand_bipolar(&rng);
                float noise_amt = cur->is_voiced ? cur->noise_mix : 1.0f;
                source += noise * noise_amt * 0.35f;
            }

            float o1 = rig_voice_formant_process(&res1, source, f1, bw1, sample_rate) * g1;
            float o2 = rig_voice_formant_process(&res2, source, f2, bw2, sample_rate) * g2;
            float o3 = rig_voice_formant_process(&res3, source, f3, bw3, sample_rate) * g3;
 ...
 * ---------------------------------------------------------------------- */
static void rig_voice_write_u32le(FILE *fp, unsigned int v) {
    unsigned char b[4] = { (unsigned char)(v), (unsigned char)(v>>8), (unsigned char)(v>>16), (unsigned char)(v>>24) };
    fwrite(b, 1, 4, fp);
}
static void rig_voice_write_u16le(FILE *fp, unsigned short v) {
    unsigned char b[2] = { (unsigned char)(v), (unsigned char)(v>>8) };
    fwrite(b, 1, 2, fp);
}

RIGCOM_PUBLIC int rig_voice_write_wav(const float *pcm, int sample_count, float sample_rate, FILE *fp) {
    ...
    fwrite("RIFF", 1, 4, fp);
    rig_voice_write_u32le(fp, 36 + data_bytes);
    fwrite("WAVE", 1, 4, fp);
    fwrite("fmt ", 1, 4, fp);
    rig_voice_write_u32le(fp, 16);
    rig_voice_write_u16le(fp, 1);       /* PCM */
    rig_voice_write_u16le(fp, 1);       /* mono */
    rig_voice_write_u32le(fp, (unsigned int)sample_rate);
    rig_voice_write_u32le(fp, byte_rate);
    rig_voice_write_u16le(fp, 2);       /* block align */
    rig_voice_write_u16le(fp, 16);      /* bits per sample */
    fwrite("data", 1, 4, fp);
    rig_voice_write_u32le(fp, data_bytes);
    ...
    for (int i = 0; i < sample_count; i++) {
        ...
        rig_voice_write_u16le(fp, (unsigned short)v);
    }
    return 0;
}
...
RIGCOM_PUBLIC int rig_voice_script_save(const RigVoiceScript *script, FILE *fp) {
...
RIGCOM_PUBLIC int rig_voice_script_load(RigVoiceScript *script, FILE *fp) {
    if (!script || !fp) return -1;
