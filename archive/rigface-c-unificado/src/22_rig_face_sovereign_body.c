/* ==========================================================================
 * 22_rig_face_sovereign_body.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : nested/Face_Pose_Body/rig_face_sovereign_body.c
 * Copias fundidas: 5
 * Funciones      : 20      Unidades injertadas: 0      Variantes: 0
 *
...
static void set_joint(RigSovSkeleton *s,size_t i,const char *name,int parent,RigSovVec3 p){
...
static void skin_single(RigSovVertex *v,unsigned j){memset(v->joints,0,sizeof(v->joints));memset(v->weights,0,sizeof(v->weights));v->joints[0]=(uint16_t)j;v->weights[0]=1.0f;}
static void skin_pair(RigSovVertex *v,unsigned a,unsigned b,float t){t=clampf(t,0,1);memset(v->joints,0,sizeof(v->joints));memset(v->weights,0,sizeof(v->weights));v->joints[0]=(uint16_t)a;v->joints[1]=(uint16_t)b;v->weights[0]=1-t;v->weights[1]=t;}

static int append_ellipsoid(RigSovMesh *m,RigSovVec3 c,RigSovVec3 r,unsigned rings,unsigned seg,unsigned joint){
    size_t base=m->vertex_count;unsigned y,x;int rc;
    if(rings<3||seg<6)return RIG_SOV_EINVAL;
    rc=rig_sov_mesh_reserve(m,base+(size_t)(rings+1)*(seg+1),m->triangle_count+(size_t)rings*seg*2);if(rc)return rc;
    for(y=0;y<=rings;y++){
        float v=(float)y/rings,phi=(float)M_PI*v;
        for(x=0;x<=seg;x++){
            float u=(float)x/seg,th=(float)(2.0*M_PI)*u;
            RigSovVertex q;RigSovVec3 n=v3(sinf(phi)*cosf(th),cosf(phi),sinf(phi)*sinf(th));memset(&q,0,sizeof(q));
    ...
    for(y=0;y<rings;y++)for(x=0;x<seg;x++){
        uint32_t a=(uint32_t)(base+y*(seg+1)+x),b=a+1,c0=(uint32_t)(base+(y+1)*(seg+1)+x),d=c0+1;
...
static int append_capsule(RigSovMesh *m,RigSovVec3 a,RigSovVec3 b,float ra,float rb,unsigned rings,unsigned seg,unsigned ja,unsigned jb){
    ...
    for(y=0;y<=rings;y++){
        ...
        for(x=0;x<=seg;x++){
            float q=(float)(2.0*M_PI)*x/seg,cs=cosf(q),sn=sinf(q);RigSovVec3 radial=add3(mul3(u,cs),mul3(v,sn));RigSovVertex z;memset(&z,0,sizeof(z));
    ...
    for(y=0;y<rings;y++)for(x=0;x<seg;x++){uint32_t p=(uint32_t)(base+y*(seg+1)+x),q=p+1,r0=(uint32_t)(base+(y+1)*(seg+1)+x),s=r0+1;m->triangles[m->triangle_count++]=(RigSovTri){p,r0,q};m->triangles[m->triangle_count++]=(RigSovTri){q,r0,s};}
    return RIG_SOV_OK;
}

static int append_torso(RigSovMesh *m,const RigSovSkeleton *s,const RigSovBodyParams *p){
    unsigned rings=p->longitudinal_segments<6?6:p->longitudinal_segments,seg=p->radial_segments<12?12:p->radial_segments,y,x;size_t base=m->vertex_count;int rc;
    ...
    for(y=0;y<=rings;y++){
        ...
        for(x=0;x<=seg;x++){
            float u=(float)x/seg,th=(float)(2.0*M_PI)*u,cs=cosf(th),sn=sinf(th);RigSovVertex z;memset(&z,0,sizeof(z));
    ...
    for(y=0;y<rings;y++)for(x=0;x<seg;x++){uint32_t a=(uint32_t)(base+y*(seg+1)+x),b=a+1,c=(uint32_t)(base+(y+1)*(seg+1)+x),d=c+1;m->triangles[m->triangle_count++]=(RigSovTri){a,c,b};m->triangles[m->triangle_count++]=(RigSovTri){b,c,d};}
    return RIG_SOV_OK;
}
...
int rig_sov_generate_complete_body(const RigSovBodyParams *in,RigSovMesh *mesh,RigSovSkeleton *skeleton){
    ...
    p.muscularity=clampf(p.muscularity,0,1);p.body_fat=clampf(p.body_fat,0,1);p.sex_factor=clampf(p.sex_factor,0,1);p.age_factor=clampf(p.age_factor,0,1);
...
int rig_sov_generate_head_from_reconstruction(const RigSovReconParams *p,unsigned rings,unsigned sectors,RigSovMesh *mesh){
    unsigned y,x;size_t base;int rc;float fw,fh,depth;
    ...
    rc=rig_sov_mesh_reserve(mesh,(size_t)(rings+1)*(sectors+1),(size_t)rings*sectors*2);if(rc)return rc;base=0;
    for(y=0;y<=rings;y++){
        float v=(float)y/rings,phi=(float)M_PI*v,ny=cosf(phi),sy=sinf(phi);
        for(x=0;x<=sectors;x++){
            float u=(float)x/sectors,th=(float)(2*M_PI)*u,nx=sy*cosf(th),nz=sy*sinf(th);RigSovVertex q;float jawT=clampf((-ny-0.05f)/0.85f,0,1),jawScale=1.0f-jawT*(1.0f-clampf(p->jaw_width/fw,0.45f,1.15f));float z=nz*depth*0.5f;
            memset(&q,0,sizeof(q));
    ...
    for(y=0;y<rings;y++)for(x=0;x<sectors;x++){uint32_t a=(uint32_t)(base+y*(sectors+1)+x),b=a+1,c=(uint32_t)(base+(y+1)*(sectors+1)+x),d=c+1;mesh->triangles[mesh->triangle_count++]=(RigSovTri){a,c,b};mesh->triangles[mesh->triangle_count++]=(RigSovTri){b,c,d};}
