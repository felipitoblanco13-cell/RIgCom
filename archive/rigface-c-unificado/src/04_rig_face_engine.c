/* ==========================================================================
 * 04_rig_face_engine.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : nested/rig_face_v2_bridge/rig_face_engine.c
 * Copias fundidas: 6
 * Funciones      : 76      Unidades injertadas: 44      Variantes: 44
 *
...
static  int arena_reset(void){ g_arena.used = 0;
    return 0; /* 0=OK, -1=ENOMEM o fallo de arena */
}
...
/* [fusion] global void__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:26 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_arena_init)(void__v2) = arena_init;

/* [fusion] global void__v3 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:44 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_arena_reset)(void__v3) = arena_reset;

/* [fusion] global void__v4 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:64 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_edge_table_clear)(void__v4) = edge_table_clear;

/* [fusion] global depth__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:236 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_sculpt_sink)(RigFaceMesh *mesh, Vec3f center, float radius, float depth__v2) = sculpt_sink;

/* [fusion] global height__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:262 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_sculpt_raise)(RigFaceMesh *mesh, Vec3f center, float radius, float height__v2) = sculpt_raise;

/* [fusion] global amount__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:275 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_sculpt_push)(RigFaceMesh *mesh, Vec3f center, float radius, Vec3f direction, float amount__v2) = sculpt_push;

/* [fusion] global zones__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:925 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_facs_compute_zone_centers)(const RigFaceMesh *mesh, Vec3f zones__v2[12]) = facs_compute_zone_centers;

/* [fusion] global RIG_FACE_BONES__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1114 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_bones_from_params)(const RigFaceParams *p, BoneDef defs[RIG_FACE_BONES__v2]) = bones_from_params;

/* [fusion] global tail__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1142 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_bone_build_bind)(RigFaceBone *b, Vec3f head, Vec3f tail__v2) = bone_build_bind;

/* [fusion] global mode__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1391 :: dato global divergente; ausente en la copia canonica */
void (*rigpub_rig_face_engine_raster_tri)(const Vec3f *p0, const Vec3f *p1, const Vec3f *p2, const Vec2f *uv0, const Vec2f *uv1, const Vec2f *uv2, const Vec3f *n0, const Vec3f *n1, const Vec3f *n2, uint8_t *out_rgb, uint32_t w, uint32_t h, int mode__v2) = raster_tri;

/* [fusion] function rig_face_build_neck__v2 <- nested/Face_Pose_Body/rig_face_engine.c:782 :: cuerpo divergente; ausente en la copia canonica */
int rig_face_build_neck__v2(RigFaceMesh *mesh, const RigFaceParams *p){
    float sy      = p->cranium_height * 0.5f;
    float sz      = p->cranium_depth  * 0.5f;
    float jaw_bot = -sy * 0.85f;
    float neck_bot = jaw_bot - p->neck_length;
...
/* [fusion] function rig_face_project_to_skull__v2 <- nested/Face_Pose_Body/rig_face_engine.c:1694 :: cuerpo divergente; ausente en la copia canonica */
int rig_face_project_to_skull__v2(RigFaceMesh *mesh){
    if (!mesh || !mesh->verts || mesh->n_verts == 0) return -1;
...
/* [fusion] function arena_init__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:18 :: cuerpo divergente; ausente en la copia canonica */
static void arena_init__v2(void) {
    if (!g_arena.base) {
        g_arena.base     = (uint8_t*)malloc(ARENA_SIZE);
        g_arena.capacity = ARENA_SIZE;
        g_arena.used     = 0;
...
/* [fusion] function arena_alloc__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:30 :: cuerpo divergente; ausente en la copia canonica */
static void* arena_alloc__v2(size_t sz) {
    sz = (sz + 15) & ~15UL;
    if (g_arena.used + sz > g_arena.capacity) return NULL;
    void *ptr = g_arena.base + g_arena.used;
    g_arena.used += sz;
...
/* [fusion] function arena_reset__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:42 :: cuerpo divergente; ausente en la copia canonica */
static void arena_reset__v2(void) { g_arena.used = 0; }

/* [fusion] function edge_table_clear__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:59 :: cuerpo divergente; ausente en la copia canonica */
static void edge_table_clear__v2(void) {
    memset(g_edge_table, 0, sizeof(g_edge_table));
    g_edge_pool_idx = 0;
}

/* [fusion] function edge_get_or_create__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:68 :: cuerpo divergente; ausente en la copia canonica */
static uint32_t edge_get_or_create__v2(RigFaceMesh *mesh,
                                    uint32_t a, uint32_t b) {
    if (a > b) { uint32_t t = a; a = b; b = t; }
    uint32_t h = (a * 2654435761u ^ b * 2246822519u) & (EDGE_TABLE_SIZE - 1);
    for (EdgeEntry *e = g_edge_table[h]; e; e = e->next) {
...
/* [fusion] function rig_face_build_base_sphere__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:122 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_build_base_sphere__v2(RigFaceMesh *mesh) {
    arena_init();
    mesh->n_verts = 12;
    mesh->n_tris  = 20;
    mesh->verts   = (RigFaceVertex*)arena_alloc(sizeof(RigFaceVertex) * RIG_FACE_VERTS_MAX);
...
/* [fusion] function rig_face_subdivide_catmull_clark__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:144 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_subdivide_catmull_clark__v2(RigFaceMesh *mesh, uint32_t iterations) {
    for (uint32_t iter = 0; iter < iterations; iter++) {
        edge_table_clear();
        uint32_t old_n_tris = mesh->n_tris;
        RigFaceTri *old_tris = (RigFaceTri*)arena_alloc(sizeof(RigFaceTri) * old_n_tris);
...
/* [fusion] function rig_face_apply_cranial_deform__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:189 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_apply_cranial_deform__v2(RigFaceMesh *mesh, const RigFaceParams *p) {
    float sx = p->cranium_width  * 0.5f;
    float sy = p->cranium_height * 0.5f;
    float sz = p->cranium_depth  * 0.5f;
...
/* [fusion] function sculpt_sink__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:226 :: cuerpo divergente; ausente en la copia canonica */
static void sculpt_sink__v2(RigFaceMesh *mesh, Vec3f center,
                         float radius, float depth) {
    for (uint32_t i = 0; i < mesh->n_verts; i++) {
        Vec3f v  = mesh->verts[i].pos;
        float w  = radial_influence(v, center, radius);
...
/* [fusion] function sculpt_raise__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:240 :: cuerpo divergente; ausente en la copia canonica */
static void sculpt_raise__v2(RigFaceMesh *mesh, Vec3f center,
                          float radius, float height) {
    for (uint32_t i = 0; i < mesh->n_verts; i++) {
        Vec3f v = mesh->verts[i].pos;
        float w = radial_influence(v, center, radius);
...
/* [fusion] function sculpt_push__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:266 :: cuerpo divergente; ausente en la copia canonica */
static void sculpt_push__v2(RigFaceMesh *mesh, Vec3f center,
                         float radius, Vec3f direction, float amount) {
    for (uint32_t i = 0; i < mesh->n_verts; i++) {
        Vec3f v = mesh->verts[i].pos;
        float w = radial_influence(v, center, radius);
...
/* [fusion] function rig_face_sculpt_features__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:279 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_sculpt_features__v2(RigFaceMesh *mesh, const RigFaceParams *p) {
    float sx = p->cranium_width  * 0.5f;
    float sy = p->cranium_height * 0.5f;
    float sz = p->cranium_depth  * 0.5f;
...
/* [fusion] function rig_face_compute_smooth_normals__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:374 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_compute_smooth_normals__v2(RigFaceMesh *mesh) {
...
/* [fusion] function rig_face_compute_tangent_basis__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:432 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_compute_tangent_basis__v2(RigFaceMesh *mesh) {
...
/* [fusion] function rig_face_bake_vertex_color__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:537 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_bake_vertex_color__v2(RigFaceMesh *mesh) {
    const RigFaceParams *p = &mesh->params;
    float sy = p->cranium_height * 0.5f;
    float sz = p->cranium_depth  * 0.5f;
    float sx = p->cranium_width  * 0.5f;
...
/* [fusion] function rig_face_unwrap_uv_seams__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:584 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_unwrap_uv_seams__v2(RigFaceMesh *mesh) {
...
/* [fusion] function rig_face_export_glsl_uniforms__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:664 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_export_glsl_uniforms__v2(const RigFaceMesh *mesh, char *buf, size_t sz) {
    const RigSkinMaterial *m = &mesh->material;
    snprintf(buf, sz,
        "// RigCom Face PBR Uniforms\n"
        "uniform vec3  u_baseColor     = vec3(%.4f, %.4f, %.4f);\n"
...
/* [fusion] function rig_face_create__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:692 :: cuerpo divergente; ausente en la copia canonica */
RigFaceMesh* rig_face_create__v2(const RigFaceParams *params, uint32_t subdiv_level) {
    arena_init();
    if (subdiv_level < 2) subdiv_level = 2;
    if (subdiv_level > 5) subdiv_level = 5;
    ...
    rig_face_build_base_sphere(mesh);
    rig_face_subdivide_catmull_clark(mesh, subdiv_level);
    rig_face_apply_cranial_deform(mesh, &mesh->params);
    rig_face_sculpt_features(mesh, &mesh->params);
    rig_face_compute_smooth_normals(mesh);
    rig_face_unwrap_uv_seams(mesh);
...
/* [fusion] function rig_face_destroy__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:734 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_destroy__v2(RigFaceMesh *mesh) {

    (void)mesh;
    arena_reset();
}

/* [fusion] function rig_face_build_neck__v3 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:740 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_build_neck__v3(RigFaceMesh *mesh, const RigFaceParams *p) {
    float sy      = p->cranium_height * 0.5f;
    float sz      = p->cranium_depth  * 0.5f;
    float jaw_bot = -sy * 0.85f;
    float neck_bot = jaw_bot - p->neck_length;
...
/* [fusion] function facs_compute_zone_centers__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:898 :: cuerpo divergente; ausente en la copia canonica */
static void facs_compute_zone_centers__v2(const RigFaceMesh *mesh, Vec3f zones[12]) {
    const RigFaceParams *p = &mesh->params;
    float sx = p->cranium_width  * 0.5f;
    (void)sx;
    float sy = p->cranium_height * 0.5f;
...
/* [fusion] function rig_face_build_facs_shapes__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:929 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_build_facs_shapes__v2(RigFaceMesh *mesh) {
    const RigFaceParams *p = &mesh->params;
    float sx = p->cranium_width * 0.5f;
...
/* [fusion] function rig_face_apply_expression__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:984 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_apply_expression__v2(RigFaceMesh *mesh, const float weights[RIG_FACE_BLEND_SHAPES]) {
...
/* [fusion] function rig_face_solve_psd__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1003 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_solve_psd__v2(RigFaceMesh *mesh) {
...
/* [fusion] function bones_from_params__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1048 :: cuerpo divergente; ausente en la copia canonica */
static void bones_from_params__v2(const RigFaceParams *p, BoneDef defs[RIG_FACE_BONES]) {
    float sx = p->cranium_width  * 0.5f;
    (void)sx;
    float sy = p->cranium_height * 0.5f;
    float sz = p->cranium_depth  * 0.5f;
...
/* [fusion] function bone_build_bind__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1118 :: cuerpo divergente; ausente en la copia canonica */
static void bone_build_bind__v2(RigFaceBone *b, Vec3f head, Vec3f tail) {
    b->head = head;
    b->tail = tail;
...
/* [fusion] function rig_face_bind_skeleton__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1146 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_bind_skeleton__v2(RigFaceMesh *mesh) {
    const RigFaceParams *p = &mesh->params;
    float sx = p->cranium_width  * 0.5f;
    (void)sx;
...
/* [fusion] function rig_face_skin_dual_quaternion__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1285 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_skin_dual_quaternion__v2(RigFaceMesh *mesh) {
...
/* [fusion] function raster_tri__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1332 :: cuerpo divergente; ausente en la copia canonica */
static void raster_tri__v2(const Vec3f *p0, const Vec3f *p1, const Vec3f *p2,
                        const Vec2f *uv0, const Vec2f *uv1, const Vec2f *uv2,
                        const Vec3f *n0,  const Vec3f *n1,  const Vec3f *n2,
                        uint8_t *out_rgb, uint32_t w, uint32_t h, int mode) {
...
/* [fusion] function rig_face_generate_normal_map__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1395 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_generate_normal_map__v2(const RigFaceMesh *mesh,
                                   uint8_t *out_rgb, uint32_t w, uint32_t h) {
    if (!out_rgb || w == 0 || h == 0) return;
...
/* [fusion] function rig_face_generate_sss_map__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1414 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_generate_sss_map__v2(const RigFaceMesh *mesh,
                                uint8_t *out_rgb, uint32_t w, uint32_t h) {
    if (!out_rgb || w == 0 || h == 0) return;
    memset(out_rgb, 80, w * h * 3);
    for (uint32_t t = 0; t < mesh->n_tris; t++) {
...
/* [fusion] function rig_face_project_to_skull__v3 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_engine.c:1641 :: cuerpo divergente; ausente en la copia canonica */
void rig_face_project_to_skull__v3(RigFaceMesh *mesh) {
    if (!mesh || !mesh->verts || mesh->n_verts == 0) return;
