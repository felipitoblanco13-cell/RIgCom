/* ==========================================================================
 * 01_rig_face_cloth.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : src_raw/rig_face_cloth-1.c
 * Copias fundidas: 1
 * Funciones      : 17      Unidades injertadas: 0      Variantes: 0
 *
...
static RigClothVec3 rc_add(RigClothVec3 a, RigClothVec3 b) { RigClothVec3 r={a.x+b.x,a.y+b.y,a.z+b.z}; return r; }
static RigClothVec3 rc_sub(RigClothVec3 a, RigClothVec3 b) { RigClothVec3 r={a.x-b.x,a.y-b.y,a.z-b.z}; return r; }
static RigClothVec3 rc_scale(RigClothVec3 a, float s) { RigClothVec3 r={a.x*s,a.y*s,a.z*s}; return r; }
static float rc_dot(RigClothVec3 a, RigClothVec3 b) { return a.x*b.x+a.y*b.y+a.z*b.z; }
static RigClothVec3 rc_cross(RigClothVec3 a, RigClothVec3 b) {
    RigClothVec3 r = { a.y*b.z-a.z*b.y, a.z*b.x-a.x*b.z, a.x*b.y-a.y*b.x }; return r;
}
static float rc_len(RigClothVec3 a) { return sqrtf(rc_dot(a,a)); }
static RigClothVec3 rc_norm(RigClothVec3 a) {
    float l = rc_len(a);
    if (l < 1e-8f) { RigClothVec3 z = {0,1,0}; return z; }
    return rc_scale(a, 1.0f / l);
}
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_cloth_patch_create_grid(RigClothPatch *cloth, int rows, int cols,
    ...
    RigClothVec3 r_unit = rc_norm(right_axis);
    RigClothVec3 d_unit = rc_norm(down_axis);

    for (int row = 0; row < rows; row++) {
        for (int col = 0; col < cols; col++) {
            int idx = row * cols + col;
            RigClothVec3 p = rc_add(origin_mm, rc_add(rc_scale(r_unit, col * spacing_mm), rc_scale(d_unit, row * spacing_mm)));
            cloth->pos[idx] = p;
    ...
    #define RIG_CLOTH_ADD_CONSTRAINT(I,J,TYPE,COMPL) do { \
        ...
        k->rest_length_mm = rc_len(rc_sub(cloth->pos[(I)], cloth->pos[(J)])); \
    } while (0)
...
RIGCOM_PUBLIC int rig_cloth_pin_add(RigClothPatch *cloth, int particle_index, RigClothVec3 world_pos_mm) {
    if (!cloth) return -1;
...
RIGCOM_PUBLIC int rig_cloth_pin_update_position(RigClothPatch *cloth, int pin_index, RigClothVec3 world_pos_mm) {
 ...
 * ---------------------------------------------------------------------- */
static unsigned int rig_cloth_hash_cell(int cx, int cy, int cz) {
    unsigned int h = (unsigned int)(cx * 73856093) ^ (unsigned int)(cy * 19349663) ^ (unsigned int)(cz * 83492791);
    return h % RIG_CLOTH_HASH_BUCKETS;
}

static void rig_cloth_self_collision_resolve(RigClothPatch *cloth) {
    ...
    for (int i = 0; i < cloth->particle_count; i++) {
        ...
        unsigned int h = rig_cloth_hash_cell(cx, cy, cz);
    ...
    for (int i = 0; i < cloth->particle_count; i++) {
        ...
        for (int dz = -1; dz <= 1; dz++) {
            unsigned int h = rig_cloth_hash_cell(cx+dx, cy+dy, cz+dz);
            for (int b = 0; b < bucket_counts[h]; b++) {
                ...
                RigClothVec3 delta = rc_sub(cloth->pos[i], cloth->pos[j]);
                float d = rc_len(delta);
                if (d < min_dist && d > 1e-6f) {
                    float penetration = min_dist - d;
                    RigClothVec3 n = rc_scale(delta, 1.0f / d);
                    float wi = cloth->inv_mass[i], wj = cloth->inv_mass[j];
                    float wsum = wi + wj;
                    if (wsum < 1e-8f) continue;
                    RigClothVec3 correction_i = rc_scale(n, penetration * (wi / wsum));
                    RigClothVec3 correction_j = rc_scale(n, -penetration * (wj / wsum));
                    cloth->pos[i] = rc_add(cloth->pos[i], correction_i);
                    cloth->pos[j] = rc_add(cloth->pos[j], correction_j);
                }
            }
        }
    }
 ...
 * ---------------------------------------------------------------------- */
static void rig_cloth_capsule_collide_point(RigClothVec3 *p, float p_radius_mm,
                                             RigClothVec3 a, RigClothVec3 b, float cap_radius_mm) {
    RigClothVec3 ab = rc_sub(b, a);
    float ab_len2 = rc_dot(ab, ab);
    float t = (ab_len2 > 1e-8f) ? (rc_dot(rc_sub(*p, a), ab) / ab_len2) : 0.0f;
    if (t < 0.0f) t = 0.0f;
    if (t > 1.0f) t = 1.0f;
    RigClothVec3 closest = rc_add(a, rc_scale(ab, t));
    RigClothVec3 delta = rc_sub(*p, closest);
    float dist = rc_len(delta);
    float min_dist = cap_radius_mm + p_radius_mm;
    if (dist < min_dist) {
        RigClothVec3 n = (dist > 1e-6f) ? rc_scale(delta, 1.0f / dist) : (RigClothVec3){0,1,0};
        *p = rc_add(closest, rc_scale(n, min_dist));
    }
}
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_cloth_step(RigClothPatch *cloth, float dt, RigClothVec3 gravity_mm_s2,
    ...
    for (int s = 0; s < substeps; s++) {
        ...
        for (int i = 0; i < cloth->particle_count; i++) {
            ...
            cloth->vel[i] = rc_add(cloth->vel[i], rc_scale(gravity_mm_s2, sub_dt));
        }
        ...
        if (wind_strength > 0.0f) {
            for (int t = 0; t < cloth->tri_count; t++) {
                ...
                RigClothVec3 e1 = rc_sub(cloth->pos[ib], cloth->pos[ia]);
                RigClothVec3 e2 = rc_sub(cloth->pos[ic], cloth->pos[ia]);
                RigClothVec3 n = rc_cross(e1, e2);
                float area2 = rc_len(n);
                if (area2 < 1e-8f) continue;
                n = rc_scale(n, 1.0f / area2);

                RigClothVec3 face_vel = rc_scale(rc_add(rc_add(cloth->vel[ia], cloth->vel[ib]), cloth->vel[ic]), 1.0f/3.0f);
                RigClothVec3 rel_wind = rc_sub(wind_mm_s, face_vel);
                float flux = rc_dot(rel_wind, n) * area2 * 0.5f * wind_strength;
                RigClothVec3 force = rc_scale(n, flux);

                float w_a = cloth->inv_mass[ia], w_b = cloth->inv_mass[ib], w_c = cloth->inv_mass[ic];
                if (w_a > 0.0f) cloth->vel[ia] = rc_add(cloth->vel[ia], rc_scale(force, sub_dt * w_a / 3.0f));
                if (w_b > 0.0f) cloth->vel[ib] = rc_add(cloth->vel[ib], rc_scale(force, sub_dt * w_b / 3.0f));
                if (w_c > 0.0f) cloth->vel[ic] = rc_add(cloth->vel[ic], rc_scale(force, sub_dt * w_c / 3.0f));
            }
        }

        for (int i = 0; i < cloth->particle_count; i++) {
            cloth->vel[i] = rc_scale(cloth->vel[i], 1.0f - cloth->damping);
            cloth->prev_pos[i] = cloth->pos[i];
            cloth->pos[i] = rc_add(cloth->pos[i], rc_scale(cloth->vel[i], sub_dt));
        }
        ...
        for (int iter = 0; iter < solver_iterations; iter++) {
            for (int c = 0; c < cloth->constraint_count; c++) {
                ...
                RigClothVec3 delta = rc_sub(cloth->pos[k->i], cloth->pos[k->j]);
                float dist = rc_len(delta);
                if (dist < 1e-8f) continue;
                RigClothVec3 n = rc_scale(delta, 1.0f / dist);
                ...
                cloth->pos[k->i] = rc_add(cloth->pos[k->i], rc_scale(n, wi * delta_lambda));
                cloth->pos[k->j] = rc_add(cloth->pos[k->j], rc_scale(n, -wj * delta_lambda));
            }
            ...
            for (int i = 0; i < cloth->particle_count; i++) {
                for (int cap = 0; cap < capsule_count; cap++) {
                    rig_cloth_capsule_collide_point(&cloth->pos[i], cloth->particle_radius_mm,
                        capsules[cap].a, capsules[cap].b, capsules[cap].radius_mm);
                }
            }
        }

        rig_cloth_self_collision_resolve(cloth);

        for (int i = 0; i < cloth->particle_count; i++) {
            cloth->vel[i] = rc_scale(rc_sub(cloth->pos[i], cloth->prev_pos[i]), 1.0f / sub_dt);
        }
    }
    return 0;
}
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_cloth_compute_smooth_normals(const RigClothPatch *cloth, RigClothVec3 *out_normals) {
    ...
    for (int t = 0; t < cloth->tri_count; t++) {
        ...
        RigClothVec3 e2 = rc_sub(cloth->pos[ic], cloth->pos[ia]);
        RigClothVec3 n = rc_cross(e1, e2);
        out_normals[ia] = rc_add(out_normals[ia], n);
        out_normals[ib] = rc_add(out_normals[ib], n);
        out_normals[ic] = rc_add(out_normals[ic], n);
    }
    for (int i = 0; i < cloth->particle_count; i++) out_normals[i] = rc_norm(out_normals[i]);
    return 0;
}
...
RIGCOM_PUBLIC int rig_cloth_save(const RigClothPatch *cloth, FILE *fp) {
...
RIGCOM_PUBLIC int rig_cloth_load(RigClothPatch *cloth, FILE *fp) {
    if (!cloth || !fp) return -1;
