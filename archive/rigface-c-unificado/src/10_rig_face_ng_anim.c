/* ==========================================================================
 * 10_rig_face_ng_anim.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : nested/Face_Pose_Body/rig_face_ng_anim.c
 * Copias fundidas: 6
 * Funciones      : 2      Unidades injertadas: 3      Variantes: 3
 *
...
static const FacsAU_NG NG_FACS_TABLE[128] = {
    ...
    /* Entradas NG completadas: canales antes reservados pero vacíos. */
    { 0, "Neutral Baseline", "Global facial resting tone", "", "neutral calibration", 500.0f, 1500.0f, 4000.0f, 1.0f, 0.25f, 'F', false, 0.02f },
    { 3, "Central Brow Compression", "Procerus + corrugator medial", "Frontalis", "concern,focus", 24.0f, 170.0f, 460.0f, 1.0f, 0.85f, 'F', true, 0.10f },
    { 35, "Cheek Suck", "Buccinator inward", "Risorius", "speech,concentration", 40.0f, 240.0f, 700.0f, 1.0f, 0.75f, 'C', false, 0.16f },
    { 36, "Tongue Bulge Left", "Intrinsic tongue left", "Intrinsic tongue right", "speech", 25.0f, 120.0f, 320.0f, 0.0f, 0.85f, 'B', false, 0.22f },
    { 37, "Tongue Bulge Right", "Intrinsic tongue right", "Intrinsic tongue left", "speech", 25.0f, 120.0f, 320.0f, 0.0f, 0.85f, 'B', false, 0.22f },
    { 38, "Nostril Compress", "Compressor naris", "Dilator naris", "disgust,effort", 22.0f, 130.0f, 360.0f, 1.0f, 0.80f, 'N', true, 0.14f },
    { 39, "Nostril Dilate", "Dilator naris", "Compressor naris", "fear,breath", 18.0f, 110.0f, 340.0f, 1.0f, 0.85f, 'N', true, 0.15f },
    { 40, "Brow Squeeze", "Corrugator transverse", "Frontalis", "pain,effort", 18.0f, 100.0f, 300.0f, 1.0f, 0.90f, 'F', true, 0.12f },
    { 47, "Blink Half", "Orbicularis oculi palpebral", "Levator palpebrae", "speech,thought", 15.0f, 70.0f, 180.0f, 1.0f, 0.65f, 'O', true, 0.18f },
    { 48, "Blink Left", "Orbicularis oculi left", "Levator palpebrae left", "blink,wink", 8.0f, 45.0f, 140.0f, 0.0f, 1.00f, 'O', true, 0.35f },
    { 49, "Blink Right", "Orbicularis oculi right", "Levator palpebrae right", "blink,wink", 8.0f, 45.0f, 140.0f, 0.0f, 1.00f, 'O', true, 0.35f },
    { 50, "Speech Mouth Base", "Orbicularis oris + jaw group", "", "phonation", 20.0f, 100.0f, 260.0f, 1.0f, 0.70f, 'B', false, 0.08f },
    { 59, "Head Forward", "Longus capitis", "Splenius capitis", "attention,challenge", 50.0f, 300.0f, 900.0f, 1.0f, 0.55f, 'F', false, 0.10f },
    { 60, "Head Back", "Splenius capitis", "Longus capitis", "surprise,avoidance", 50.0f, 300.0f, 900.0f, 1.0f, 0.55f, 'F', false, 0.10f },
    { 67, "Eye Squint Left", "Orbicularis oculi left", "Levator palpebrae left", "focus,skepticism", 20.0f, 120.0f, 360.0f, 0.0f, 0.80f, 'O', true, 0.30f },
    { 68, "Eye Squint Right", "Orbicularis oculi right", "Levator palpebrae right", "focus,skepticism", 20.0f, 120.0f, 360.0f, 0.0f, 0.80f, 'O', true, 0.30f },
    { 69, "Upper Lid Press", "Orbicularis oculi superior", "Levator palpebrae", "strain,pain", 18.0f, 100.0f, 320.0f, 1.0f, 0.75f, 'O', true, 0.12f },
    { 71, "Lower Lid Drop Left", "Inferior tarsal muscle left", "Orbicularis oculi left", "fatigue,sadness", 35.0f, 220.0f, 700.0f, 0.0f, 0.65f, 'O', false, 0.22f },
    { 72, "Lower Lid Drop Right", "Inferior tarsal muscle right", "Orbicularis oculi right", "fatigue,sadness", 35.0f, 220.0f, 700.0f, 0.0f, 0.65f, 'O', false, 0.22f },
    { 73, "Cheek Raise Left", "Zygomaticus + orbicularis left", "Depressor anguli left", "joy asymmetry", 25.0f, 150.0f, 430.0f, 0.0f, 0.90f, 'C', true, 0.28f },
    { 74, "Cheek Raise Right", "Zygomaticus + orbicularis right", "Depressor anguli right", "joy asymmetry", 25.0f, 150.0f, 430.0f, 0.0f, 0.90f, 'C', true, 0.28f },
    { 75, "Nose Wrinkle Left", "Levator labii alaeque nasi left", "Nasalis left", "disgust asymmetry", 20.0f, 110.0f, 330.0f, 0.0f, 0.85f, 'N', true, 0.30f },
    { 76, "Nose Wrinkle Right", "Levator labii alaeque nasi right", "Nasalis right", "disgust asymmetry", 20.0f, 110.0f, 330.0f, 0.0f, 0.85f, 'N', true, 0.30f },
    { 77, "Lip Press Left", "Orbicularis oris left", "Risorius left", "restraint asymmetry", 28.0f, 160.0f, 480.0f, 0.0f, 0.80f, 'B', true, 0.28f },
    { 78, "Lip Press Right", "Orbicularis oris right", "Risorius right", "restraint asymmetry", 28.0f, 160.0f, 480.0f, 0.0f, 0.80f, 'B', true, 0.28f },
    { 79, "Jaw Slide Micro", "Lateral pterygoid", "Contralateral pterygoid", "speech,uncertainty", 20.0f, 110.0f, 300.0f, 0.0f, 0.55f, 'B', true, 0.20f },
    { 91, "Forehead Vein", "Frontal vascular plexus", "", "effort,heat", 250.0f, 1800.0f, 9000.0f, 1.0f, 0.55f, 'F', false, 0.03f },
    { 92, "Tear Meniscus Left", "Lacrimal system left", "", "sadness,irritation", 300.0f, 1800.0f, 8000.0f, 0.0f, 0.90f, 'O', false, 0.08f },
    { 93, "Tear Meniscus Right", "Lacrimal system right", "", "sadness,irritation", 300.0f, 1800.0f, 8000.0f, 0.0f, 0.90f, 'O', false, 0.08f },
    { 94, "Tear Flow Left", "Lacrimal punctum left", "", "crying", 500.0f, 2500.0f, 12000.0f, 0.0f, 1.00f, 'O', false, 0.10f },
    { 95, "Tear Flow Right", "Lacrimal punctum right", "", "crying", 500.0f, 2500.0f, 12000.0f, 0.0f, 1.00f, 'O', false, 0.10f },
    { 96, "Sweat Forehead", "Eccrine glands forehead", "", "heat,stress", 1200.0f, 6000.0f, 30000.0f, 1.0f, 0.90f, 'F', false, 0.02f },
    { 97, "Sweat Upper Lip", "Eccrine glands upper lip", "", "heat,stress", 900.0f, 4500.0f, 24000.0f, 1.0f, 0.90f, 'B', false, 0.02f },
    { 98, "Sebum T Zone", "Sebaceous glands T-zone", "", "time,heat", 2500.0f, 15000.0f, 90000.0f, 1.0f, 0.75f, 'N', false, 0.01f },
    { 99, "Skin Pallor", "Dermal vascular constriction", "", "fear,shock", 180.0f, 1200.0f, 7000.0f, 1.0f, 0.85f, 'C', true, 0.05f },
...
static const Phoneme_NG NG_PHONEME_TABLE[60] = {
    ...
    /* Estados multilingües y articulaciones de transición completados. */
    {51,"ʔ",  "glottal stop",  17, 0.02f,0.10f,0.30f,0.00f,0.45f,0.55f,0.00f,0.0f,PHON_GLOTTAL},
    {52,"ʀ",  "rouge (uvular)",11, 0.30f,0.25f,0.35f,0.12f,0.45f,0.80f,0.15f,0.0f,PHON_VELAR},
    {53,"ç",  "ich (de)",     12, 0.12f,0.10f,0.55f,0.00f,0.80f,0.35f,0.35f,0.0f,PHON_PALATAL},
    {54,"ɣ",  "lago (sp)",    19, 0.15f,0.10f,0.45f,0.00f,0.30f,0.85f,0.20f,0.0f,PHON_VELAR},
    {55,"β",  "saber (sp)",    6, 0.08f,0.30f,0.35f,0.08f,0.50f,0.55f,0.20f,0.0f,PHON_BILABIAL},
    {56,"ɾ",  "pero (sp)",    11, 0.18f,0.15f,0.50f,0.08f,0.70f,0.35f,0.20f,0.0f,PHON_ALVEOLAR},
    {57,"ts", "pizza",        9, 0.07f,0.08f,0.68f,0.00f,0.82f,0.18f,0.55f,0.0f,PHON_ALVEOLAR},
    {58,"dz", "adze",         9, 0.07f,0.08f,0.68f,0.00f,0.82f,0.18f,0.55f,0.0f,PHON_ALVEOLAR},
    {59,"ǀ",  "dental click", 7, 0.02f,0.20f,0.30f,0.00f,0.90f,0.05f,0.20f,0.0f,PHON_DENTAL}
};
...
/* [fusion] global NG_FACS_TABLE__v2 <- nested/rig_face_ng_v24/tmp/rig_ng_package/src/rig_face_ng_anim.c:102 :: dato global divergente; ausente en la copia canonica */
static const FacsAU_NG NG_FACS_TABLE__v2[128] = {
    /* ── AU 0-9: Frente y cejas ─────────────────────────────── */
    { 1, "Inner Brow Raise",
      "Frontalis (pars medialis)", "Corrugator supercilii",
      "sadness,fear,surprise", 20.f, 150.f, 400.f, 1.0f, 1.0f, 'F', true,  0.12f },
...
/* [fusion] global NG_PHONEME_TABLE__v2 <- nested/rig_face_ng_v24/tmp/rig_ng_package/src/rig_face_ng_anim.c:318 :: dato global divergente; ausente en la copia canonica */
static const Phoneme_NG NG_PHONEME_TABLE__v2[60] = {
    /* ── Vocales inglés/español ───────────────────────────────── */
    {0, "iː", "see/si",       0, 0.20f, 0.05f, 0.85f, 0.00f, 0.90f, 0.10f, 0.60f, 0.0f, PHON_VOWEL},
    {1, "ɪ",  "sit",          0, 0.22f, 0.05f, 0.80f, 0.00f, 0.80f, 0.15f, 0.50f, 0.0f, PHON_VOWEL},
    {2, "e",  "bed/ve",       1, 0.35f, 0.05f, 0.70f, 0.00f, 0.70f, 0.25f, 0.40f, 0.0f, PHON_VOWEL},
...
/* [fusion] function rig_face_ng_anim_generate__v2 <- nested/16_FACE_NG-1/16_FACE_NG/src/rig_face_ng.c:3736 :: cuerpo divergente; absorbido de rig_face_ng */
int rig_face_ng_anim_generate__v2(const RigFaceNGAnimCtx *ctx, RigArtResultNG *out)
{
    if (!ctx || !out) return -1;
