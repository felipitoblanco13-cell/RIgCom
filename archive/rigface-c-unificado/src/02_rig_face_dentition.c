/* ==========================================================================
 * 02_rig_face_dentition.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : src_raw/rig_face_dentition.c
 * Copias fundidas: 1
 * Funciones      : 9      Unidades injertadas: 0      Variantes: 0
 *
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_dentition_arcade_init_standard(RigDentalArcade *arc) {
    if (!arc) return -1;
 ...
 * ---------------------------------------------------------------------- */
static void rig_dent_arch_position(float arch_width_mm, float arch_depth_mm,
 ...
 * ---------------------------------------------------------------------- */
static float rig_dent_lerp(float a, float b, float t) { return a + (b - a) * t; }

static float rig_dent_profile_radius(const RigToothClassProfile *prof, float h) {
    ...
    return rig_dent_lerp(prof->radius_profile[i0], prof->radius_profile[i1], smooth_t);
}

static int rig_dent_generate_tooth(const RigToothInstance *tooth, RigDentalMesh *mesh,
    ...
    for (int ring = 0; ring <= RIG_DENT_CROWN_RINGS; ring++) {
        ...
        float base_r = rig_dent_profile_radius(prof, h);
        ...
        for (int seg = 0; seg < RIG_DENT_RADIAL_SEGMENTS; seg++) {
            ...
            float h_eps = 0.02f;
            float r_next = rig_dent_profile_radius(prof, h + h_eps > 1.0f ? 1.0f : h + h_eps);
            ...
            mesh->stain_color[vi][0] = rig_dent_lerp(enamel_rgb[0], tooth->stain_color_rgb[0], stain_amt);
            mesh->stain_color[vi][1] = rig_dent_lerp(enamel_rgb[1], tooth->stain_color_rgb[1], stain_amt);
            mesh->stain_color[vi][2] = rig_dent_lerp(enamel_rgb[2], tooth->stain_color_rgb[2], stain_amt);
    ...
    float cervical_r = rig_dent_profile_radius(prof, 0.0f);
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_dentition_generate_arcade(const RigDentalArcade *arc, RigDentalMesh *out_mesh) {
    ...
    for (int i = 0; i < RIG_DENT_TOOTH_COUNT; i++) {
        ...
        rig_dent_arch_position(width, depth, t->arch_slot, t->is_right_side, &pos, &yaw);
        ...
        int rc = rig_dent_generate_tooth(t, out_mesh, pos, yaw, enamel_copy);
...
RIGCOM_PUBLIC int rig_dentition_generate_shader(char *out_glsl, int max_len) {
...
RIGCOM_PUBLIC int rig_dentition_arcade_save(const RigDentalArcade *arc, FILE *fp) {
    if (!arc || !fp) return -1;
...
RIGCOM_PUBLIC int rig_dentition_arcade_load(RigDentalArcade *arc, FILE *fp) {
    if (!arc || !fp) return -1;
