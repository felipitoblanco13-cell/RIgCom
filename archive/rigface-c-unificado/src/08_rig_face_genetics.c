/* ==========================================================================
 * 08_rig_face_genetics.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : src_raw/rig_face_genetics-1.c
 * Copias fundidas: 1
 * Funciones      : 19      Unidades injertadas: 0      Variantes: 0
 *
 ...
 * ---------------------------------------------------------------------- */
static unsigned int rig_genetics_xorshift32(unsigned int *state) {
...
static float rig_genetics_rand01(unsigned int *state) {
    return (float)(rig_genetics_xorshift32(state) & 0x00FFFFFFu) / (float)0x01000000u;
}

static float rig_genetics_randn(unsigned int *state) {
    /* Box-Muller */
    float u1 = rig_genetics_rand01(state);
    float u2 = rig_genetics_rand01(state);
    if (u1 < 1e-7f) u1 = 1e-7f;
    return sqrtf(-2.0f * logf(u1)) * cosf(6.283185307f * u2);
}

static float rig_genetics_clampf(float v, float lo, float hi) {
    if (v < lo) return lo;
    if (v > hi) return hi;
    return v;
}
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_genetics_basis_init(RigGenomeBasisSet *set) {
    if (!set) return -1;
    memset(set, 0, sizeof(*set));
    return 0;
}

RIGCOM_PUBLIC int rig_genetics_basis_add(RigGenomeBasisSet *set, const char *name,
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_genetics_inherit(const RigGenomeIndividual *parent_a,
    ...
    for (int i = 0; i < RIG_GENOME_TRAIT_COUNT; i++) {
        ...
        float roll = rig_genetics_rand01(&rng);
        float base_value;
        ...
        if (codominance > 0.6f) {
            ...
            float w = rig_genetics_clampf(0.5f + (roll - 0.5f) * 0.4f, 0.0f, 1.0f);
        ...
        float mutated = base_value + rig_genetics_randn(&rng) * mutation_sigma;
        ...
        float dom_child = 0.5f * (da + db) + rig_genetics_randn(&rng) * 0.03f;
        out_child->dominance[i] = rig_genetics_clampf(dom_child, 0.0f, 1.0f);
    }
    return 0;
}
 ...
 * Covarianza
 * ---------------------------------------------------------------------- */
static void rig_genetics_compute_mean(const RigGenomeBasisSet *set, RigGenomeVector *out_mean) {
...
static void rig_genetics_compute_covariance(const RigGenomeBasisSet *set,
 ...
 * ---------------------------------------------------------------------- */
static void rig_genetics_jacobi_eigen(float a[RIG_GENOME_TRAIT_COUNT][RIG_GENOME_TRAIT_COUNT],
                                       float eigenvalues[RIG_GENOME_TRAIT_COUNT],
...
static void rig_genetics_sort_eigen_desc(float eigenvalues[RIG_GENOME_TRAIT_COUNT],
                                          float eigenvectors[RIG_GENOME_TRAIT_COUNT][RIG_GENOME_TRAIT_COUNT],
                                          int n) {
    for (int i = 0; i < n - 1; i++) {
        int best = i;
...
RIGCOM_PUBLIC int rig_genetics_pca_compute(const RigGenomeBasisSet *set, RigGenomePCASpace *out_space) {
    ...
    rig_genetics_compute_mean(set, &out_space->mean);

    static float cov[RIG_GENOME_TRAIT_COUNT][RIG_GENOME_TRAIT_COUNT];
    rig_genetics_compute_covariance(set, &out_space->mean, cov);

    rig_genetics_jacobi_eigen(cov, out_space->eigenvalues, out_space->eigenvectors,
                               RIG_GENOME_TRAIT_COUNT, 100);
    rig_genetics_sort_eigen_desc(out_space->eigenvalues, out_space->eigenvectors, RIG_GENOME_TRAIT_COUNT);
...
RIGCOM_PUBLIC int rig_genetics_pca_project(const RigGenomePCASpace *space,
                                            const RigGenomeVector *vec,
...
RIGCOM_PUBLIC int rig_genetics_pca_reconstruct(const RigGenomePCASpace *space,
                                                const float *coeffs,
...
RIGCOM_PUBLIC int rig_genetics_pca_sample_random(const RigGenomePCASpace *space,
                                                  unsigned int seed,
                                                  float sigma_scale,
                                                  RigGenomeVector *out_vec) {
    ...
    for (int c = 0; c < space->component_count; c++) {
        ...
        coeffs[c] = rig_genetics_randn(&rng) * std_dev * sigma_scale;
    }
    return rig_genetics_pca_reconstruct(space, coeffs, out_vec);
}
...
RIGCOM_PUBLIC int rig_genetics_basis_save(const RigGenomeBasisSet *set, FILE *fp) {
    if (!set || !fp) return -1;
...
RIGCOM_PUBLIC int rig_genetics_basis_load(RigGenomeBasisSet *set, FILE *fp) {
    if (!set || !fp) return -1;
...
RIGCOM_PUBLIC int rig_genetics_vector_to_text(const RigGenomeVector *vec, FILE *fp) {
    if (!vec || !fp) return -1;
...
RIGCOM_PUBLIC int rig_genetics_vector_from_text(RigGenomeVector *vec, FILE *fp) {
    if (!vec || !fp) return -1;
    char line[128];
