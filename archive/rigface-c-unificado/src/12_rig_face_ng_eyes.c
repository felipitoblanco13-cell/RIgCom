/* ==========================================================================
 * 12_rig_face_ng_eyes.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : nested/Face_Pose_Body/rig_face_ng_eyes.c
 * Copias fundidas: 6
 * Funciones      : 9      Unidades injertadas: 4      Variantes: 4
 *
...
/* [fusion] function eye_dynamics_init__v2 <- nested/rig_face_v2_bridge/rig_face_ng_eyes.c:493 :: cuerpo divergente; ausente en la copia canonica */
int eye_dynamics_init__v2(EyeDynamicsNG *e, float light_level)
{
    if (!e) return -1;
    memset(e, 0, sizeof(*e));
    e->light_level     = light_level;
...
/* [fusion] function eye_dynamics_init__v3 <- nested/rig_face_ng_v24/tmp/rig_ng_package/src/rig_face_ng_eyes.c:487 :: cuerpo divergente; ausente en la copia canonica */
static void eye_dynamics_init__v3(EyeDynamicsNG *e, float light_level)
{
    if (!e) return;
    memset(e, 0, sizeof(*e));
    e->light_level     = light_level;
...
/* [fusion] function eye_dynamics_update__v2 <- nested/rig_face_ng_v24/tmp/rig_ng_package/src/rig_face_ng_eyes.c:501 :: cuerpo divergente; ausente en la copia canonica */
void eye_dynamics_update__v2(EyeDynamicsNG *e, float dt)
{
    if (!e || dt <= 0.0f) return;
    e->time_acc  += dt;
    e->micro_time+= dt;
...
/* [fusion] function eye_dynamics_set_gaze__v2 <- nested/rig_face_ng_v24/tmp/rig_ng_package/src/rig_face_ng_eyes.c:584 :: cuerpo divergente; ausente en la copia canonica */
void eye_dynamics_set_gaze__v2(EyeDynamicsNG *e, float h, float v)
{
    if (!e) return;
    float dist = sqrtf((h-e->gaze_h)*(h-e->gaze_h) + (v-e->gaze_v)*(v-e->gaze_v));
    e->gaze_target_h = clamp(h, -30.0f, 30.0f);
