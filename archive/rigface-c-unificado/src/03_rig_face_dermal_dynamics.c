/* ==========================================================================
 * 03_rig_face_dermal_dynamics.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : src_raw/rig_face_dermal_dynamics.c
 * Copias fundidas: 1
 * Funciones      : 14      Unidades injertadas: 0      Variantes: 0
 *
 ...
 * ---------------------------------------------------------------------- */
static float rig_dermal_clamp01(float v) {
    if (v < 0.0f) return 0.0f;
    if (v > 1.0f) return 1.0f;
    return v;
}

static float rig_dermal_lerp(float a, float b, float t) {
    return a + (b - a) * t;
}

static float rig_dermal_approach(float current, float target, float dt, float tau) {
 ...
 * Inicializacion
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_dermal_state_init(RigDermalState *st) {
    if (!st) return -1;
 ...
 * Simulacion temporal
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_dermal_update(RigDermalState *st, float dt) {
    ...
    float blush_target = rig_dermal_clamp01(0.6f * shame_component + 0.5f * arousal_pos
                          + 0.08f * (st->core_temp_c - 37.0f));

    int shock_mode = (st->emotion_arousal > 0.7f && st->emotion_valence < -0.5f);
    float pallor_target;
    if (shock_mode) {
        pallor_target = rig_dermal_clamp01(0.5f + 0.5f * st->emotion_arousal);
    } else {
        float cold_component = rig_dermal_clamp01((15.0f - st->ambient_temp_c) / 20.0f);
        pallor_target = rig_dermal_clamp01(cold_component * 0.7f);
    }

    float heat_component = rig_dermal_clamp01((st->ambient_temp_c - 26.0f) / 12.0f);
    float sweat_target = rig_dermal_clamp01(0.7f * heat_component + 0.6f * st->exertion
                          + 0.25f * arousal_pos);

    float cold_shiver = rig_dermal_clamp01((12.0f - st->ambient_temp_c) / 15.0f);
    float fear_chill   = shock_mode ? 0.6f : 0.0f;
    float goosebump_target = rig_dermal_clamp01(cold_shiver + fear_chill - 0.5f * sweat_target);
    ...
    st->blush_level     = rig_dermal_approach(st->blush_level,     blush_target,     dt, blush_tau);
    st->pallor_level    = rig_dermal_approach(st->pallor_level,    pallor_target,    dt, pallor_tau);
    st->sweat_level     = rig_dermal_approach(st->sweat_level,     sweat_target,     dt, sweat_tau);
    st->goosebump_level = rig_dermal_approach(st->goosebump_level, goosebump_target, dt, goose_tau);
...
static RigWoundStage rig_dermal_wound_stage(const RigDermalWound *w) {
...
RIGCOM_PUBLIC int rig_dermal_wound_add(RigDermalState *st, float u, float v,
    ...
    w->depth_mm = depth_mm;
    w->age_days = 0.0f;
    w->keloid_bias = rig_dermal_clamp01(keloid_bias);
    w->active = 1;
    return st->wound_count - 1;
}

RIGCOM_PUBLIC int rig_dermal_wound_eval(const RigDermalWound *w,
                                         float *out_color_delta_rgb,
                                         float *out_height_mm) {
    if (!w || !out_color_delta_rgb || !out_height_mm) return -1;
    RigWoundStage stage = rig_dermal_wound_stage(w);
    switch (stage) {
        ...
        case RIG_WOUND_GRANULATING: {
            ...
            out_color_delta_rgb[0] = rig_dermal_lerp(0.35f, 0.18f, t);
            out_color_delta_rgb[1] = rig_dermal_lerp(0.08f, 0.06f, t);
            out_color_delta_rgb[2] = rig_dermal_lerp(0.08f, 0.06f, t);
            *out_height_mm = rig_dermal_lerp(0.5f, 0.15f, t) * w->depth_mm;
            break;
        }
        ...
        default: {
            ...
            out_color_delta_rgb[0] = rig_dermal_lerp(-0.15f, 0.05f, hyper);
            out_color_delta_rgb[1] = rig_dermal_lerp(-0.10f, -0.02f, hyper);
            out_color_delta_rgb[2] = rig_dermal_lerp(-0.10f, -0.02f, hyper);
...
RIGCOM_PUBLIC int rig_dermal_region_color_delta(const RigDermalState *st, int region_idx,
                                                 float *out_rgb) {
...
RIGCOM_PUBLIC int rig_dermal_region_surface(const RigDermalState *st, int region_idx,
    ...
    *out_wetness   = rig_dermal_clamp01(st->sweat_level * st->region_map.sweat_density[region_idx]);
    *out_goosebump = rig_dermal_clamp01(st->goosebump_level * st->region_map.piloerection_density[region_idx]);
    return 0;
}
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_dermal_wounds_rasterize(const RigDermalState *st,
    ...
    for (int i = 0; i < st->wound_count; i++) {
        ...
        float color_delta[3];
        float height_mm;
        rig_dermal_wound_eval(w, color_delta, &height_mm);
        ...
        for (int y = y0; y <= y1; y++) {
            for (int x = x0; x <= x1; x++) {
                ...
                for (int c = 0; c < 3; c++) {
                    ...
                    float blended = rig_dermal_lerp(base, color_delta[c], falloff);
                ...
                float blended_h = rig_dermal_lerp(base_h, height_mm, falloff);
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_dermal_generate_shader(RigDermalArtResult *out) {
    if (!out) return -1;
...
RIGCOM_PUBLIC int rig_dermal_state_save(const RigDermalState *st, FILE *fp) {
    if (!st || !fp) return -1;
...
RIGCOM_PUBLIC int rig_dermal_state_load(RigDermalState *st, FILE *fp) {
    if (!st || !fp) return -1;
    RigDermalFileHeader hdr;
