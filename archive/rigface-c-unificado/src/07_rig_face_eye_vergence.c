/* ==========================================================================
 * 07_rig_face_eye_vergence.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : src_raw/rig_face_eye_vergence-1.c
 * Copias fundidas: 1
 * Funciones      : 8      Unidades injertadas: 0      Variantes: 0
 *
...
static RigVec3 rig_vergence_sub(RigVec3 a, RigVec3 b) {
    RigVec3 r = { a.x - b.x, a.y - b.y, a.z - b.z }; return r;
}
static float rig_vergence_len(RigVec3 v) {
    return sqrtf(v.x * v.x + v.y * v.y + v.z * v.z);
}
static RigVec3 rig_vergence_norm(RigVec3 v) {
    float l = rig_vergence_len(v);
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_vergence_compute(const RigHeadFrame *head, RigVec3 fixation_point_world,
    ...
    RigVec3 to_target_left  = rig_vergence_sub(fixation_point_world, eye_left);
    RigVec3 to_target_right = rig_vergence_sub(fixation_point_world, eye_right);

    RigVec3 dir_left  = rig_vergence_norm(to_target_left);
    RigVec3 dir_right = rig_vergence_norm(to_target_right);
    ...
    float dist_left  = rig_vergence_len(to_target_left);
    float dist_right = rig_vergence_len(to_target_right);
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC float rig_vergence_pupil_radius_mm(float ambient_lux, float focus_distance_mm,
...
RIGCOM_PUBLIC int rig_vergence_generate_shader(char *out_glsl, int max_len) {
...
RIGCOM_PUBLIC int rig_vergence_sample_save(const RigVergenceResult *res, float timestamp_s, FILE *fp) {
    if (!res || !fp) return -1;
    RigVergenceSample s;
...
RIGCOM_PUBLIC int rig_vergence_sample_load(RigVergenceResult *res, float *out_timestamp_s, FILE *fp) {
    if (!res || !fp) return -1;
    RigVergenceSample s;
