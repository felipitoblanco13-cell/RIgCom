/* ==========================================================================
 * 30_rig_face_v2_bridge.c   —   RIGFACE :: C UNIFICADO
 *
 * Copia canonica : nested/rig_face_v2_bridge/rig_face_v2_bridge.c
 * Copias fundidas: 5
 * Funciones      : 13      Unidades injertadas: 5      Variantes: 5
 *
...
/* ========================================================================
 * SECCION DE FUSION — unidades reales de las otras 4 copias de este
 * modulo, ausentes en la copia canonica. Se incorporan integras.
 * ======================================================================== */

/* [fusion] function rigart_renderer_html_v4__v2 <- nested/Face_Pose_Body/rig_face_v2_bridge.c:398 :: cuerpo divergente; ausente en la copia canonica */
int rigart_renderer_html_v4__v2(const RIgArtRendererCtxV4 *ctx, RigArtResultV4 *out)
{
    if (!ctx || !out) return -1;
    rigart_v4_init_result(out);
...
/* [fusion] function b4_str__v2 <- src_raw/rig_face_v2_bridge.c:43 :: cuerpo divergente; ausente en la copia canonica */
static void   b4_str__v2(const char *j, const char *k, char *out, size_t sz)
{
    if (!j || !out || !sz) return;
    const char *p = strstr(j, k);
    if (!p) return;
...
/* [fusion] function rigart_face_session_v2_init__v2 <- src_raw/rig_face_v2_bridge.c:60 :: cuerpo divergente; ausente en la copia canonica */
void rigart_face_session_v2_init__v2(RIgArtFaceSessionV2 *s)
{
    if (!s) return;
    memset(s, 0, sizeof(*s));
    const RigFaceArchetype *_def = rig_archetype_get(RIG_ARCH_AGE_YOUNG_25);
...
/* [fusion] function rigart_face_session_v2_destroy__v2 <- src_raw/rig_face_v2_bridge.c:71 :: cuerpo divergente; ausente en la copia canonica */
void rigart_face_session_v2_destroy__v2(RIgArtFaceSessionV2 *s)
{
    if (!s) return;
    if (s->mesh) { rig_face_v2_destroy(s->mesh); s->mesh = NULL; }
    rigart_v4_free_result(&s->last_result);
...
/* [fusion] function rigart_face_v2_dispatch__v2 <- src_raw/rig_face_v2_bridge.c:78 :: cuerpo divergente; ausente en la copia canonica */
void rigart_face_v2_dispatch__v2(WsServer *srv, const char *cmd,
                              const char *payload,
                              RIgArtFaceSessionV2 *session)
{
