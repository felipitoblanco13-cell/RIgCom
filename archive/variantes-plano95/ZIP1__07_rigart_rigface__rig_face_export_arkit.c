...
static float rig_arkit_clamp01__rig_dup_f32c8dec(float v) { return v < 0.0f ? 0.0f : (v > 1.0f ? 1.0f : v); }
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_arkit_compute__rig_dup_50584047(float (*au_lookup)(void *userdata, int au_number), void *userdata,
    ...
    for (int i = 0; i < RIG_ARKIT_AU_TABLE_COUNT; i++) {
        ...
        float contribution = rig_arkit_clamp01__rig_dup_f32c8dec(au_val) * m->weight;
    ...
    out->weights[RIG_ARKIT_eyeLookInLeft]   = rig_arkit_clamp01__rig_dup_f32c8dec(gl_yaw_n);
    out->weights[RIG_ARKIT_eyeLookOutLeft]  = rig_arkit_clamp01__rig_dup_f32c8dec(-gl_yaw_n);
    out->weights[RIG_ARKIT_eyeLookOutRight] = rig_arkit_clamp01__rig_dup_f32c8dec(gr_yaw_n);
    out->weights[RIG_ARKIT_eyeLookInRight]  = rig_arkit_clamp01__rig_dup_f32c8dec(-gr_yaw_n);

    out->weights[RIG_ARKIT_eyeLookUpLeft]    = rig_arkit_clamp01__rig_dup_f32c8dec(-gl_pitch_n);
    out->weights[RIG_ARKIT_eyeLookDownLeft]  = rig_arkit_clamp01__rig_dup_f32c8dec(gl_pitch_n);
    out->weights[RIG_ARKIT_eyeLookUpRight]   = rig_arkit_clamp01__rig_dup_f32c8dec(-gr_pitch_n);
    out->weights[RIG_ARKIT_eyeLookDownRight] = rig_arkit_clamp01__rig_dup_f32c8dec(gr_pitch_n);
    #undef RIG_ARKIT_GAZE_RANGE_DEG

    /* mandibula: pose continua, no proviene de una tabla AU */
    out->weights[RIG_ARKIT_jawOpen] = rig_arkit_clamp01__rig_dup_f32c8dec(jaw_open01);
    out->weights[RIG_ARKIT_jawLeft] = rig_arkit_clamp01__rig_dup_f32c8dec(-jaw_lateral_norm);
    out->weights[RIG_ARKIT_jawRight] = rig_arkit_clamp01__rig_dup_f32c8dec(jaw_lateral_norm);
    out->weights[RIG_ARKIT_jawForward] = rig_arkit_clamp01__rig_dup_f32c8dec(jaw_forward01);
 ...
 * ---------------------------------------------------------------------- */
RIGCOM_PUBLIC int rig_arkit_export_json__rig_dup_376cb25f(const RigARKitFrame *frame, char *out_json, int max_len) {
...
RIGCOM_PUBLIC int rig_arkit_frame_save__rig_dup_43571010(const RigARKitFrame *frame, float timestamp_s, FILE *fp) {
...
RIGCOM_PUBLIC int rig_arkit_frame_load__rig_dup_46b27448(RigARKitFrame *frame, float *out_timestamp_s, FILE *fp) {
    if (!frame || !fp) return -1;
    RigARKitFileFrame f;
