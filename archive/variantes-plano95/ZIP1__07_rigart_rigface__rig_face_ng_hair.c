...
static int hair_strand_init__rig_dup_537d47d6(
    HairStrand *strand,
    ...
    strand->particles   = (HairParticle*)rl_calloc(
    ...
    strand->n_len = n_segs;
    strand->len_c = (HairLengthConstraint*)rl_calloc(
    ...
    strand->bend_c = (HairBendConstraint*)rl_calloc(
...
int hair_sim_step__rig_variant_77881476(HairSimSystem *sys, float dt)
{
    if (!sys || !sys->strands || dt <= 0.0f) return 0;
    sys->wind_time += dt;
...
int hair_sim_free__rig_variant_4c0dde09(HairSimSystem *sys)
{
    if (!sys) return -1;
    for (int i = 0; i < sys->n_strands; i++) {
        rl_free(sys->strands[i].particles);
        rl_free(sys->strands[i].len_c);
        rl_free(sys->strands[i].bend_c);
    }
    rl_free(sys->strands);
    rl_memset(sys, 0, sizeof(*sys));
    return 0;
}
 ...
 * ═══════════════════════════════════════════════════════════════ */
int rig_face_ng_hair_shader__rig_dup_e6a97af1(const RigHairNGCtx *ctx, RigArtResultNG *out)
{
    if (!ctx || !out) return -1;

    char *frag = (char*)rl_malloc(HAIR_BUF);
    char *vert = (char*)rl_malloc(HAIR_BUF / 2);
    char *js   = (char*)rl_malloc(HAIR_JS_BUF);
    if (!frag || !vert || !js) { rl_free(frag); rl_free(vert); rl_free(js); return -1; }
