...
/* RIGCOM_PUBLIC_STATIC_EXPORT_BEGIN: clamp -> rigpub_rig_face_ng_eyes_clamp */
...
static void eye_dynamics_init(EyeDynamicsNG *e, float light_level)
{
    if (!e) return;
    memset(e, 0, sizeof(*e));
...
/* RIGCOM_PUBLIC_STATIC_EXPORT_BEGIN: eye_dynamics_init -> rigpub_rig_face_ng_eyes_eye_dynamics_init */
void (*rigpub_rig_face_ng_eyes_eye_dynamics_init)(EyeDynamicsNG*e, float light_level) = eye_dynamics_init;
/* RIGCOM_PUBLIC_STATIC_EXPORT_END */

void eye_dynamics_update(EyeDynamicsNG *e, float dt)
{
    if (!e || dt <= 0.0f) return;
    e->time_acc  += dt;
    e->micro_time+= dt;
...
void eye_dynamics_set_gaze(EyeDynamicsNG *e, float h, float v)
{
    if (!e) return;
...
void hair_sim_step(HairSimSystem *sys, float dt)
{
...
void hair_sim_free(HairSimSystem *sys)
{
    if (!sys) return;
...
/* RIGCOM_PUBLIC_STATIC_EXPORT_BEGIN: hair_strand_init -> rigpub_rig_face_ng_hair_hair_strand_init */
...
/* ════════════════════════════════════════════════════════════════
 * rig_face_ng_anim_generate() — Función principal de generación
 * ════════════════════════════════════════════════════════════════ */
int rig_face_ng_anim_generate(const RigFaceNGAnimCtx *ctx, RigArtResultNG *out)
{
    if (!ctx || !out) return -1;
 ...
 * ════════════════════════════════════════════════════════════════ */
void rig_face_ng_params_default(RigFaceNGParams *p)
{
    if (!p) return;
    memset(p, 0, sizeof(*p));
...
void rig_face_ng_result_free(RigArtResultNG *r)
{
    if (!r) return;
