...
/* ════════════════════════════════════════════════════════════════
 * rig_face_ng_generate_html__rig_dup_0e104823() — Genera el HTML standalone completo
 * ════════════════════════════════════════════════════════════════ */
int rig_face_ng_generate_html__rig_dup_0e104823(const RigFaceNGParams *params,
...
/* ════════════════════════════════════════════════════════════════
 * rig_face_ng_assemble__rig_dup_7377e16c() — Función principal de ensamblaje
 * ════════════════════════════════════════════════════════════════ */
int rig_face_ng_assemble__rig_dup_7377e16c(const RigFaceNGParams *params,
                          const RigFaceNGRenderCtx *render,
                          RigArtResultNG *out)
{
    if (!params || !out) return -1;

    char *html = (char*)rl_malloc(HTML_BUF);
    if (!html) return -1;

    int len = rig_face_ng_generate_html__rig_dup_0e104823(params, render, html, HTML_BUF);
    if (len <= 0) { rl_free(html); return -1; }
...
/* ════════════════════════════════════════════════════════════════
 * rig_face_ng_params_default__rig_variant_9817e526() — Defaults de producción
 * ════════════════════════════════════════════════════════════════ */
int rig_face_ng_params_default__rig_variant_9817e526(RigFaceNGParams *p)
{
    if (!p) return 0;
    rl_memset(p, 0, sizeof(*p));
    rl_snprintf(p->name, sizeof(p->name), "Avatar NG Default");
    rl_snprintf(p->id,   sizeof(p->id),   "avatar_ng_%016llx", (unsigned long long)(uintptr_t)p);
    p->phi_seed         = NG_PHI;
...
int rig_face_ng_result_free__rig_variant_28e6ada8(RigArtResultNG *r)
{
    if (!r) return 0;
    rl_free(r->glsl_vert); rl_free(r->glsl_frag);
    rl_free(r->js);        rl_free(r->html);
    rl_free(r->json_params);rl_free(r->c_params);
    rl_memset(r, 0, sizeof(*r));
    return 0;
}
